...
## 下次从这里继续（截至 2026-09-10 第六十三轮收工）

### 2026-09-10（第六十三轮）：常备文档过——「绿灯只回答它自己的问题」

⚠️ **这一轮最重要的产出是 `docs/TESTING.md`。**

仓库里有 **17 个** `test:*` / `check:*` 命令，**没有任何地方写过每一个是干什么的**。
这不是文档缺口，这就是 bug 上线的方式：第 62 轮我跑 `npm run test:notifications`
看到 23 项全绿，就认为通知没问题——而那套测试跑的是 SQL，
**在结构上根本看不见缺失的翻译键**。绿灯的意思是「查询是对的」，
被我读成了「通知能用」。

新文档给每个检查写了两列：**它证明了什么** 和 **它在结构上看不见什么**。
第二列才是有用的那一列。并写下由此得出的五条规则，
每条都对应这两轮踩到的一个具体的坑：

1. 绿灯只回答它自己的问题，不回答你的问题
2. 跨层的改动需要跨层的检查（通知是「行 + 路由 + 轮询 + 角标 + 翻译串」五层，
   测到了四层，坏的是第五层）
3. 走查要两个人、分别驱动（第 46 轮的教训）
4. 等那个东西，不要等一段时间（靠运气过一次比每次都挂更糟）
5. 探针 bug 看起来和产品 bug 一模一样（先查探针再改产品）

还有一节写明**目前完全没有自动检查的地方**——
未写下来的缺口和已覆盖的区域看起来是一样的。

⚠️ **README 严重过时。** 标题写着「3D Portfolio」、正文写着
「A full-stack **personal portfolio** for Right」——这在 2026-09-07 之后
已经不成立了；功能列表还写着联系表单写进 `data/messages.jsonl`
（那只是**没有数据库时的回落**，生产走 PostgreSQL）。
已改成市集的真实描述，并加了文档索引与测试小节。
索引里每个文件和每条命令都**逐个核实过存在**。

完成内容：

- `docs/TESTING.md`：新增
- `README.md`：标题与描述改为市集；功能列表重写；新增文档索引与测试小节
- `CLAUDE.md`：第 6 条扩展为「改到哪些地方要跑哪些检查」，
  并把 `docs/TESTING.md` 加进「开工前必读」
- `docs/ARCHITECTURE.md`：通知一节改为指向两个检查，并写明二者各自看不见什么
...
- README 文档索引：7 个文件 + 5 条命令 + 4 个 `OPERATIONS_*`，**逐个核实存在**
- 本轮无代码改动、无部署、无数据库写入
...
- 需要主体资质：平台收款
- 全站 4 件作品、1 个创作者
- `docs/TESTING.md` 末尾列出的三处无自动检查区域（裁剪框/编辑器、发信、一切视觉判断）

---

## 2026-09-10（第六十二轮）

### 2026-09-10（第六十二轮）：在真浏览器里走通知全链，抓到一个真 bug

⚠️ **每一条通知的「发生了什么」那一行，一直是空的。**
在每个页面、对每个账号、从上线那一刻起。

`kindLabel` 拼字典键时**没把首字母大写**：`order:placed` 拼出
`notificationKindorderPlaced`，任何字典里都没有这个键，
查不到就回落成空字符串。标题照常显示，所以面板看起来完全正常。
**不报错、不告警、没有测试会红。**

只有在浏览器里走一遍才能发现它:
`verify-notifications.mjs` 验的是查询，看不见缺失的 i18n 键；
build 看不见运行时拼出来的字符串。

⚠️ **两个账号必须分开驱动，这是整件事的关键。**
第 46 轮那次「陌生人全流程走查」本该抓到「卖家没被通知」，却没抓到——
因为是一个人同时拿着买卖双方的账号走的，**他永远已经知道了**。
创作者「得知」某事，只有在买家是另一个人时才算数。

`scripts/verify-notification-journey.mjs`（`npm run test:notification-journey`）
就是那次走查，并且留下来：一次性 PostgreSQL 集群 + 真服务器 + 真构建前端 +
**两个独立浏览器上下文里的两个账号** + 一件真实定价的作品 +
真的点那个购买按钮。9 项检查。

⚠️ **它先抓到了我自己的两个探针 bug，然后才抓到真 bug**，
而这两个看起来都**和产品坏了一模一样**：
1. 页面上第一个 `.primary-action` **不是**购买按钮
2. `PUT /api/account/works/:id` 是 **PATCH**，PUT 返回 404

两个都导致作品保持免费、按钮显示「Download free」、订单从未产生。
如果我当时直接下结论「下单坏了」，就会去改根本没坏的代码。

⚠️ 面板文本那条断言原本**在点击后立刻读**，
第一次靠运气过了、第二次挂了——**这比每次都挂更糟**。改成等列表真的到达。

完成内容：

- `src/components/NotificationBell.jsx`：修复 `kindLabel` 首字母大写
- `scripts/verify-notification-journey.mjs`：新增端到端走查
- `eslint.config.js`：给这一个驱动浏览器的脚本加窄配置块
  （不放宽整个 `scripts/`，否则另外二十个纯 Node 脚本里的 `document` 笔误会被掩盖）
- `package.json`：注册 `test:notification-journey`
...
- `npm run build` / `lint`：通过
- `npm run test:unit`：348 通过
- **`npm run test:notification-journey`：9 项全过**
  （「买家下单 → 创作者被告知，无需自己去看」「面板写出作品名」
  「写出发生了什么」「打开即清空角标」「刷新后仍是清空」
  「创作者看得到这笔销售」「确认收款后买家被告知」
  「站点公告能到达一个已清空的铃铛」）
- VPS 部署：成功；**本地与线上构建产物内容哈希一致**（`index-C5ffba6N.js`），
  确认线上跑的就是修复后的代码
- 接口验证：`CLAUDE.md` 第 9 条完整清单全过，含 301、`qrUrl`/`methods`（各 0）、
  thumbnail（4/4 ✓）、4 个通知接口均 401、`internalId` 出现 0 次

备份路径：应用见部署日志；本轮无数据库写入（走查用的是一次性集群）
...
你圈了创作者主页上的「关于/经历」块，说这些应该让创作者自由更改，
另外要一个通知模块。**这一轮做完了前一半并上线**，通知模块是下一轮。

⚠️ **那块内容原本是站主专属的。** 它来自 `content.js`，渲染在
`isSiteOwner` 判断后面，**别的创作者根本没有**——一个 300 字的 bio
就是别人在这个「请你来卖作品」的站上能说的全部。

现在 `visitor_users` 上四个可空列（`about` / `highlights` / `skills` /
`experience`），一个渲染器给所有人，账号设置里一个编辑器，
和资料表单一起保存。**没写过的创作者渲染出空**，
而不是一列空标题。

⚠️ **单语言**（你选的）。`content.js` 保留 zh/en/ja 三语是因为那是一个人
手工维护的；要求每个创作者把自我介绍写三遍，结果是一个填了、两个空着，
主页在三种语言里有两种是空的。**代价**：站主原有的 en/ja 文案不再渲染
（`content.js` 一个字没删，随时可以贴回来），所以现在英文界面下会看到
英文标题配中文正文。

⚠️ **这些字段全是陌生人写的、渲染在公开页面上**，所以每一项都有边界
（长度、条数、形状），在 `server/creatorProfile.js` 里，21 条测试。
两个具体的坑：
- `String(null)` 是 `'null'`、`String({})` 是 `'[object Object]'`，
  两者都会**原样出现在别人主页上**。所以非字符串一律变成 `''`，不做强转。
- 工具箱按大小写不敏感去重——「Blender」和「blender」并排看起来像渲染 bug，
  而不像两个工具。

⚠️ **字数计数器不是装饰**：服务端是静默截断的，看不到上限的创作者
会以「最后一句话消失了」的方式发现它。

完成内容：

- `server/creatorProfile.js`：新增，边界与归一化
- `server/postgres/schema.js`：`visitor_users` 加 4 个可空列（增量迁移）
- `server/postgres/{mappers,authStore}.js`：读写四个新列
- `server/index.js`：`normalizeAccountProfile` 接上
- `src/components/CreatorProfileContent.jsx`：新增，公开渲染器（所有人）
- `src/components/CreatorProfileEditor.jsx`：新增，账号设置里的编辑器
- `src/pages/{PublicProfilePage,AccountPage}.jsx`：接线；去掉 `isSiteOwner` 特例
- `src/index.css` / `src/lib/i18n.js`：样式与 23 键 × 3 语言
- `scripts/migrate-owner-profile-content.mjs`：新增，把站主内容迁进他自己的行
- `tests/unit/creator-profile.spec.js`：新增 21 条
...
- `npm run build` / `lint`：通过
- `npm run test:unit`：**348 通过**（新增 21）
- `npm run test:api:db`：**196 通过**
- `npm run test:schema-migration`：通过，干净且幂等
- 浏览器实测编辑器（stub 会话，不涉及任何线上凭证，PUT 被拦截未发送）：
  1440 与 440 两个宽度，输入的段落换行、卡片、工具箱、经历
  **全部以正确形状进入请求体**；440px 下零溢出、控件不超出容器
  （最宽控件 348px = 容器 348px）
- VPS 部署：成功
- 数据库写入：先备份（`mrright-portfolio-20260910-043809.dump`），
  再迁移站主内容 **1 行**（按主键单行 UPDATE），`content.js` 未改动
- 接口验证：`CLAUDE.md` 第 9 条完整清单全过
- 线上实测 `/u/mrright`（1440 与 440）：新区块与页面容器**像素级对齐**，
  2 段介绍、3 张卡片、7 个工具、3 条经历，0 破图、0 溢出

备份路径：

- 应用：见部署日志　- 数据库：`mrright-portfolio-20260910-043809.dump`

待办事项：

- ⚠️ **通知模块还没做**（你要的三种都要）：
  1. 站点公告（管理员发，所有人看，顶栏铃铛 + 未读）
  2. 系统提醒（平台发给个人）—— **这是目前市集真正缺的**：
     创作者现在没有任何途径知道有陌生人下了单
  3. 创作者公告（创作者发给访客，显示在自己主页）
- 站主的 en/ja 自我介绍目前不渲染（见上），要恢复得手工贴进编辑器
- 需要你的账号：外部 uptime 服务
- 需要主体资质：平台收款
- 全站 4 件作品、1 个创作者
...
### 2026-09-10（第五十九轮）：删掉那 3.4 MB（你明确授权后）

`CLAUDE.md` 第 3 条是「不要删除上传文件」，你说「你可以删去」，
所以这一次是**在明确授权下**破例，且只删被点名的那三个。

删前做的事：

1. **重新跑了一次孤儿扫描**——名单可能在我问和你答之间变了（上一轮
   就发生过：我以为要修的封面，其实你已经自己重传了）。名单没变。
2. **把数据库当前引用的每一张图列出来**，确认三个待删文件都不在其中。
3. **记录了指纹**（大小 + sha256 前 16 位）。
4. **查了硬链接数**：三个文件的 `links` 是 3–4，也就是说数据同时存在于
   3 个备份目录里。删掉 `/opt` 下这一份只是减一个链接，
   **数据不会消失**，从任一备份 `cp` 回来即可。

删除的三个（只删了这三个，备份目录一个没碰）：

- `banners/1788677108192-8ee87dbb5182.jpg`（1.72 MB，被裁剪后的 webp 取代）
- `avatars/1788677164353-43732322ef6f.png`（1.67 MB，同上）
- `banners/1788976106246-…-banner.webp`（45 KB，我第 56 轮回填产出的
  2.5:1 废弃文件——`cover` 比例 bug 的遗留物）

`public/uploads/` 从 24 MB 降到 21 MB，文件数 16 → 13。

完成内容：

- 无代码改动；一次人工复核过的文件删除操作

commit：无（本轮是运维操作与核验）

验证结果：

- 孤儿扫描：**No orphaned files found（13 个文件）** —— 删前是 3 个
- 备份可恢复性：三个备份目录里头像原图仍是 1752127 bytes，可随时取回
- 数据库引用的图片：头像 200 / 封面 200 / 作品图 8 个全部 200
- 接口验证：`CLAUDE.md` 第 9 条完整清单全过，含 301 与 `qrUrl`/`methods`（各 0）
- 浏览器实测 `/u/mrright`：11 张图 **0 破图**，6 个 `/uploads` 请求全部 200

⚠️ 一个探针写法上的坑：我一开始用「有没有 `<img>` 的 naturalWidth > 0」
判断封面是否渲染，得到 `bannerPainted: false` 差点当成删坏了——
**封面是 CSS 背景图，根本不是 `<img>`**，那种探法测不到它。
改成读 `backgroundImage` 加看网络请求，确认 200、元素 1150x224。

⚠️ **恢复窗口是有限的**：备份保留最新 3 份，再部署 3 次这些原图就会
随最老的备份一起被裁掉。要长期留档得现在另存。

待办事项：

- 需要你的账号才能做：外部 uptime 服务
- 需要主体资质才能做：平台收款（主体 + 持牌分账 + 每个创作者 KYC，见 ADR §6）
- 全站只有 4 件作品、1 个创作者。首页的稀薄现在只剩这一个原因——
  这不是代码能解决的。
...
### 2026-09-10（第五十八轮）：首页不再是一个人的自我介绍

你说「全都要改」，我把待办里所有我能做的都做了。

⚠️ **hero 文案换掉了。** 一个人人都能注册、发布、售卖的站，首页开口是
「你好，我是 Right」——陌生人被告知的是一个人，而不是这个站是什么，
第二个按钮还把他送去那个人的联系方式。

自我介绍没有消失，它第 51 轮就搬到 `/u/mrright` 了，**在那里是对的**。
首页现在说这个站是什么，并给出仅有的两件事：浏览，或者发布。
**hero 已经完全不接收 profile 参数**——这才是那次转型真正的形状，
而不只是措辞。

⚠️ **「首页太单调」是能量出来的，而且原因不是版块太少。**
`section-space` 是 `min-h-screen`，所以作品区不管装多少内容都占满一屏：
1440px 下实测 **900px 的版块裹着 213px 的网格**，最后一件作品到社区之间
**561px 全是空的**。四件作品的页面就该短，不该撑出来装忙。
这个类已删，空隙现在 96px。

⚠️ **社区区块原本是三个按钮通向同一个页面**（「进入社区」「查看讨论」
「分享作品」）。一个目的地三个标签，和一个点了没反应的图标是同一种不诚实。
现在它**列出真实的帖子**，每条链到自己，外加一个通向版块的链接。
往首页加密度，只有「数据库里的真实行」是值得加的。

⚠️ **hero 下面那条发布横幅也从首页去掉了**：hero 第二个按钮和顶栏
都已经在说同一件事，800px 内三个「发布」号召是同一个毛病。
`/explore` 保留（那里没有 hero 承担它）。

其它一并做掉的：

- 那张亮紫色「Creative Focus」卡片原本是 `from-royal to-lavender`，
  在青绿配近黑的站里是另一个站的组件——原模板遗留。已改成站点色调。
- **`useDeferredHero` 已删。** 它曾把 3D hero 拦在 `/projects/:slug`
  冷加载之后，实测有效（10.1s 中位数降到 7.2s）；但那条路由第 52 轮起
  返回 301，`useMatch` 永不匹配，此后一直是死的。第 53 轮特意记下来，
  就是为了今天**有意识地删掉**，而不是将来被当成新谜题重查一遍。
- About/Experience/Contact 搬走后剩的三个空 `<Suspense>` 壳。
- **目录加载失败不再伪装成「还没有作品」**：`status` 交给 `Projects`，
  失败时显示加载错误。那个变量现在有了正经用途（hero 不再需要它）。
- 删 12 个字典键、加 9 个，各三语；**563 键三语完全一致**。
  字典用法测试抓出了每一个孤儿键，也两次抓到我删多行值时只删了首行。

⚠️ **本地依赖终于和线上对齐了。** 沙箱禁网装不了新版，
所以此前**本地测试跑的是旧版 multer/body-parser/qs/sharp**（35 个生产包不符）。
这轮在 VPS 上做了一次干净的 `npm ci --omit=dev`，把差异包流回本地。
现在 327 个测试跑在**和生产完全同版本**的包上。
（剩下 21 个"不符"全是 darwin/win32/arm/musl 等其它平台的可选二进制，
linux-x64 上 npm 本来就不装，VPS 上也没有。）

完成内容：

- `src/components/HeroText.jsx`：重写为市集门面，不再依赖任何账号数据
- `src/sections/Hero.jsx`：去掉 profile/ownerHandle/status 参数
- `src/sections/Community.jsx`：重写为真实帖子列表
- `src/sections/Projects.jsx`：去掉整屏最小高度；加载失败有自己的状态
- `src/App.jsx`：删 useDeferredHero、三个空 Suspense、首页的发布横幅；status 下传
- `src/index.css`：社区列表样式；紫卡片改色；删 `.section-space` 与社区入口卡片旧样式
- `src/lib/i18n.js`：hero 与社区文案重写（-12 / +9 键 × 3 语言）
- `tests/unit/i18n-usage.spec.js`：前缀清单去掉 `communityEntry`
- `node_modules`：从 VPS 取回与 lockfile 一致的生产包
...
- `npm run build` / `lint`：通过
- `npm run test:unit`：327 通过（**跑在与线上同版本的依赖上**）
- 本地实测（1440 / 440）：无个人问候语、4 张瓦片、2 条真实帖子、
  版块间距 561px → **96px**、零横向溢出
- ⚠️ 一次差点又被截图骗到：桌面 hero 按钮在截图里是灰的——
  那是 1.6s 延迟动画拍到了中途。等动画结束后实测 `opacity: 1`、高 46px。
  和第 53 轮同一个坑，探针已改成等最后一个动画元素。
- VPS 部署：成功
- 接口验证：`CLAUDE.md` 第 9 条完整清单全过，含 301、`qrUrl`/`methods`（各 0）、
  thumbnail 检查（4/4 ✓）
- 线上实测：首页 `personalGreeting: false`、4 瓦片、2 帖子、
  hero 按钮指向 `/explore` 与 `/login`；`/u/mrright` 上个人问候语**仍在**
  （那里就该在），紫卡片已是青绿渐变；两处 0 破图、0 溢出

备份路径：

- 应用：见部署日志（deploy:vps 自动创建并保留最新 3 份）
- 数据库：本轮无写入
- `node_modules` 被替换的目录已备份到本次会话的 scratchpad（可回滚）

待办事项：

- ⚠️ **需要你一句话**：`public/uploads/` 下有 **3.4 MB 已被取代的原图**
  （两张原始头像/封面 + 一张回填废弃文件）。删除上传文件是你
  `CLAUDE.md` 第 3 条明令禁止的，所以我没动。要清就明确说「删除这三个文件」。
- 需要你的账号才能做：外部 uptime 服务
- 需要主体资质才能做：平台收款（主体 + 持牌分账 + 每个创作者 KYC，见 ADR §6）
- 全站只有 4 件作品、1 个创作者。首页的稀薄现在只剩这一个原因了——
  这不是代码能解决的。
...
### 2026-09-10（第五十七轮）：把上一轮暂停时挂着的那件事收尾

只有一件事：部署第 56 轮那个 `cover` 比例修复，然后修正线上那张 2.5:1 的封面。

⚠️ **结果是「不用改」——而且差一点就改错了。**

我原计划是：把 `banner_url` 指回原始 jpg，再重跑回填（回填会跳过已经是
`.webp` 的源）。动手前先查了一眼那行的当前值，发现文件名对不上：

- 回填生成的命名带原文件名：`…-1788677108192-8ee87dbb5182-banner.webp`
- 线上那行却是 `1788976241380-3ed9b2f4-**banner-banner**.webp`

`-banner-banner` 是**上传路由**的命名格式（`storeProfileImage` 把
profileName 同时当作源名和后缀传）。也就是说，**主人在 17:50 自己通过
新的裁剪框传了一张封面**——比我的回填晚两分钟。它是 **1200x300 = 4.00**，
30.4 KB，完全正确：客户端裁剪已经产出 4:1，服务端 cover 在一张本来就
4:1 的输入上不可能把比例弄错。

**我那套「指回原图重跑回填」会把主人自己裁的图覆盖掉**，换成对旧照片的
自动裁剪。查一眼才没干出来。

顺带这也是裁剪功能第一次有**真实用户端到端走通**的证据——
之前我只能用 stub 会话在浏览器里验。

⚠️ **孤儿扫描的补丁在真数据上验证通过**：16 个文件里只报 3 个孤儿，
**不再把作品的封面和模型误报成孤儿**（补丁前它们全都会被报出来）。
那 3 个是被派生图取代的两张原图（各 1.7 MB）加上我回填产出的那张
2.5:1 废弃文件（45 KB），合计 3.4 MB。按第 3 条规则没有删，留给你决定。

完成内容：

- 部署 `3e1be64`（`cover` 与 `withoutEnlargement` 不能叠加的修复）
- 未改任何代码；未做任何数据库写入（查证后确认不需要）

commit：无新提交（本轮是部署与核验）

验证结果：

- 部署前 `npm run build` / `lint` / `test:unit`：通过，327 通过
- 部署前 env 检查：`ADMIN_TOKEN` 与 `DATABASE_URL` 均 `[set]`（未输出值）
- VPS 部署：成功（`3e1be64` 起，实际部署到 `c12cdc0`）
- 已部署管线实测：1200x960 源图 → banner **1200x300（4.00 ✓）**、
  avatar **512x512（正方形 ✓）**
- 接口验证：`CLAUDE.md` 第 9 条完整清单全过，
  含 301、`qrUrl`/`methods`（各 0）、**新增的 thumbnail 检查（4/4 有值 ✓）**
- 线上实测 `/u/mrright`（1920 与 440）：
  `.owner-section` 与 `.public-profile-content` **左边缘与宽度完全一致**
  （1920px 下都是 left 384 / width 1152），0 破图、0 横向溢出
- 孤儿扫描（只读）：16 个文件报 3 个孤儿，作品文件不再误报

备份路径：

- 应用：见部署日志（deploy:vps 自动创建并保留最新 3 份）
- 数据库：本轮无写入，沿用 `mrright-portfolio-20260909-174809.dump`

待办事项：

- `public/uploads/` 下有 **3.4 MB 已被取代的原图**（两张原始头像/封面
  加一张回填废弃文件）。按第 3 条规则没有删；要清理的话是一次
  人工复核过的单独操作，不是给扫描脚本加个开关。
- 4. 首页仍偏单调。**图片那一半已经解决**（15.31 MB → 108.5 KB），
  剩下的一半是数据（全站 4 件作品）和 hero 文案仍是个人口吻
  （"Hi, I am Right"），需要你定。
- 个人主页那块的「Creative Focus」是亮紫色卡片（`grid-special-color`），
  和全站的青/深色调不搭——原模板留下的，要不要改由你定。
- 本地 `node_modules` 还是升级前的 multer/body-parser/qs（沙箱禁网装不了），
  本地测试跑旧版、线上跑新版。下次能联网时 `npm install` 一次。
