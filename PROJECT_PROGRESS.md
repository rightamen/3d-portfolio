...
⚠️ **瓦片带上创作者头像，并且头像和名字是同一个链接**——
点脸进作者主页正是你要的；而一个 17px 的圆圈在手机上不是能瞄准的目标，
所以名字跟它一起进链接，不是并排放着。只有主页真的可读时才是链接
（`profilePublic` 已经把访客自己的设置和管理员的封禁都折进去了），
瓦片不该把一次点击花在"该主页已隐藏"上。

⚠️ **发布界面原本是一列纯文字标题。** 没有封面图的作品在 `/explore` 上
就是一块空白瓦片，而这个列表——正是上传封面图的那个界面——
恰恰是创作者唯一看不到这件事的地方。现在没图的位置直接写「还没有封面图」。

⚠️ **两个字母的头像回落写了三遍**，一字不差，只有第二个参数的名字不同。
抽成 `src/lib/initials.js` 并配测试；测试用**三个仍然合法的变异**验证过：
只取一个字母、不再跳过空候选、两词分支不再大写——三次全部被抓到。

⚠️ **实测发现一个真问题（本轮未修，见待办）**：
目录页四张瓦片要下载 **15.31 MB 原图**当缩略图（最大一张 8.47 MB），
**头像 1.75 MB**——为了一个 17px 的圆圈。
上传的图片是按原尺寸存、按原尺寸发的，没有任何派生图。
这就是目录页看着"空/慢"的真实原因：图要几秒才画出来。
`sharp` 已经在 devDependencies 里（脚本用），做这件事要把它挪到 dependencies。

完成内容：

- `src/components/WorkCard.jsx`：创作者头像 + 名字合成一个链接
- `src/components/WorksPanel.jsx`：作品行加 4:3 缩略图与「还没有封面图」
- `src/lib/initials.js`：新增，替换三处重复实现
- `tests/unit/initials.spec.js`：新增 10 条
- `src/pages/{Account,PublicProfile}Page.jsx`、`src/components/AccountMenuBar.jsx`：改用共享实现
- `src/index.css`：`.work-tile-avatar` / `.work-tile-creator` 改成 flex 行；`.works-row-thumb*`
- `src/lib/i18n.js`：新增 `worksNoPreview` × 3 语言
...
- `npm run test:unit`：291 通过（新增 10）
- 变异测试：3 个合法变异，全部被测试抓到
- 本地实测 `/account/works`（stub 会话，不涉及任何线上凭证）：
  2 行 2 缩略图、1 张图 + 1 条「还没有封面图」、88x66（4:3）、零横向溢出
- VPS 部署：成功
- 接口验证：`CLAUDE.md` 第 9 条完整清单全过，含 301 与 `qrUrl`/`methods`（各 0）
- 线上实测 `/explore`（1440 与 440）：4 张瓦片、4 个头像、
  **4 个头像全部在 `a.work-tile-creator` 里面**、零破图、零横向溢出
...
- ⚠️ **图片没有派生图**（本轮实测）：目录页 15.31 MB、头像 1.75 MB。
  建议和你那第 1 条（头像/封面自由裁剪）**放同一轮做**——
  裁剪本来就要服务端图像处理，派生缩略图是同一套机器。
- 1. 头像与封面上传要能自由调整与裁剪 —— 见上
- 4. 首页仍偏单调。**它现在的稀薄一半是数据问题**（全站只有 4 件作品），
  另一半是 hero 文案仍是个人口吻（"Hi, I am Right"）需要你定。
  我不打算加「热门 / 编辑精选」这类把同样 4 件作品换个标题再摆一遍的板块——
  那和一个点了没反应的铃铛是同一种不诚实。
...
你要前端更像 ArtStation。离那个样子最远的三处：卡片、网格、
以及挡在两者前面的整屏 hero。

⚠️ **卡片改成瓦片**：作品图占满，价格是图上的角标，标题和作者压在底部。
没有边框、没有卡片底色、**没有摘要段落**——在网格里扫视是在看图，
每张图下面一段话正是让它变成"商品条目"而不是作品缩略图的原因。
间距收到 0.4rem，页面读起来是一面作品墙而不是一排盒子。

⚠️ **元信息只在「有悬停的设备」上浮现。**
触屏没有悬停，所以「悬停才出现的标题」等于手机用户永远读不到的标题——
`hover: hover` 以下它就待在图片下方。键盘用户走 `:focus-within`，
`prefers-reduced-motion` 下取消缩放。三条都是**量出来的**不是看出来的：
`static`/`absolute`、`opacity` 0→1、触屏 `hoverMatches: false`。

⚠️ **首页原本有自己的一套卡片**（徽章、年份、格式、摘要、技术栈、
两个按钮），和 `/explore` 那套早已漂移。现在两边共用同一个瓦片，
模型查看器移到作品页——那才是"已经选定某个作品的人"所在的地方，
而不是还在扫视网格的每一张瓦片上。`Projects.jsx` 因此少了一半。

⚠️ **hero 保留 3D**（ADR §5），但从整屏改成 68vh，落地时就能看到作品。

**这一轮我三次凭眼睛怀疑、三次被测量纠正**，值得记住：

1. 以为瓦片文字压在图上 → 量出触屏 `static`、元信息在图下方；
   看到的"重叠"是**占位图本身的内容**（本地回落数据用的是网页截图）。
2. 以为 hero 标题被顶栏遮住 → **探针测的是 `display:none` 的元素**
   （窄屏走的是另一个分支），隐藏元素的 `top` 就是 0。
   只测真正渲染的元素后，四个宽度全部 clear。
   我基于那次错误测量加的 padding **已撤销**。
3. 以为瓦片标题发灰 → 量出是 `rgb(255,255,255)` 纯白；
   是缩放后的截图骗了我。

⚠️ **测试抓到一个我引入的真回归**：重设计把空态弄丢了——
筛到一个没人发布过的分类，网格空白且没有任何提示。
是 i18n 用法测试报告那两个字符串"没被渲染"才浮出来的。已补回；
`openModelPreview` / `viewDetails` 是真死键，删掉。

⚠️ **顺带发现**：整套 hero 延迟机制（第 26–28 轮为 `/projects/:slug` 建的）
现在是**死的**——那条路由 301 了、没有任何东西链向它，
`useMatch` 永远不匹配。没有当场拔掉，留了注释，
免得以后被当成新发现重新查一遍。

完成内容：

- `src/components/WorkCard.jsx`：重写为瓦片
- `src/sections/Projects.jsx`：改用共享瓦片，去掉自有卡片与查看器（241→约 110 行）
- `src/index.css`：网格与瓦片样式（含 `hover`/`focus-within`/`reduced-motion`）
- `src/sections/Hero.jsx`：68vh
- `src/App.jsx`：回落项目补 `url`；`releaseHero` 不再解构
- `src/lib/i18n.js`：删 2 个死键 × 3 语言
...
- 浏览器实测（本地）：桌面悬停/聚焦浮现、触屏常显、440px 无横向溢出
- VPS 部署：成功
- 接口验证：`CLAUDE.md` 第 9 条完整清单全过，含 301 与 `qrUrl` 检查（0）
- 线上实测：440px 与 1280px 各 4 个瓦片、零横向溢出、零控制台错误

备份路径：

- `/opt/mrright-portfolio.backup-20260909-141552`

待办事项：

- ⚠️ **`@mrright` 的显示名仍然是 `111111`**——**现在它出现在首页每一张瓦片上**，
  比以前显眼得多。
- 顶栏还没统一：首页是汉堡+锚点，`/explore`、`/w/...`、`/u/...` 是另一套
  `auth-nav`。ArtStation 的一条常驻顶栏（logo / 浏览 / 搜索 / 上传 / 头像）
  是下一步最像它的改动。
- Hero 文案仍是个人口吻（"Hi, I am Right"），需要你定。
- hero 延迟机制是死代码，可以删（见上）。
- 升级到平台收款需要：主体 + 持牌分账 + 每个创作者 KYC（见 ADR §6）
- 仍未决：外部 uptime 服务（需要你的账号）

---

## 下次从这里继续（截至 2026-09-09 第五十二轮收工）

### 今天六轮的总览（2026-09-09，第四十七~五十二轮）

**功能面收尾，然后发现真正缺的不是功能。**

| 轮次 | 做了什么 |
|---|---|
| 47 | 后台的凭证链视图（API 早就有，管理员还在手动 curl） |
| 48 | 给管理台字典补「必须被渲染」的守卫 |
| 49 | **补上「发布作品」的界面**——它一直不存在 |
| 50 | 首页读市集目录；「你也可以发布」的号召；账号页标签写进 URL |
| 51 | 站主的 About/Experience/Contact 搬到 `/u/mrright` |
| 52 | 目录先给作品：分类说明从 1096px 收到 330px |

**今天最要紧的一件事**：第四十九轮发现**前端根本没有发布作品的入口**。
API 第三十八轮就写好了，客户端里从来没有任何代码调用过它——
站点写着「人人可上传」写了两天，而第二个创作者只能自己手敲 API。
补完之后，**一个陌生人可以走完：注册 → 选用户名 → 发布 → 填收款方式 →
被购买 → 确认到账 → 买家下载**，每一步都在浏览器里验证过。

**今天学到的四件事：**

1. ⚠️ **一个测试可能在空转，而它看起来完全正常。**
   管理台的键位守卫第一版：因为把字典文件本身扫了进去，
   每个键都被自己的定义"提到"，**加一个没人渲染的键进去照样绿**。
   是变异发现的，不是看出来的。
2. ⚠️ **动手前先量。** 第五十二轮没有凭感觉调视觉，
   而是先量出「作品区标题到第一个作品之间有 1096px」，改完再量成 330px。
   数字既是判断依据，也是验收标准。
3. ⚠️ **别在懒加载的东西前面放固定 sleep。**
   第五十一轮的浏览器测试挂在这上面——区块一直在正常渲染，是测试在掷硬币。
   等元素，不等时钟。
4. ⚠️ **`git checkout --` 恢复不了未跟踪的新文件。**
   还原变异时踩到，手工还原又留下一行不可达代码。新文件的变异要用备份文件还原。

**测试规模**：单元 277 → **281**，API-DB 契约 196（未变，今天多为前端）。

---


### 2026-09-09（第五十二轮，当天第六轮）：目录先给作品，不是先给六篇分类说明

搬走个人区块之后去线上量了首页，发现一个具体缺陷。

⚠️ **440px 下实测：作品区从 1247px 开始，第一个作品卡片在 2343px——
中间 1096px 全是六个分类的说明段落**，而整页 6046px。
也就是**约五分之一的页面，是挡在人们要看的东西前面的解释**。
原因是选「全部」（默认）时，`asset-category-strip` 会把六个分类全渲染出来。

在作品集上，「我是这样组织作品的」是一句关于手艺的陈述；
在目录上，它是第一件商品前面的六篇小作文。
现在只渲染**被选中**的那个分类的说明；筛选芯片仍然带着全部分类和数量，
所以信息没有隐藏，只是移到一次点击之后。

**改后线上实测：间距 330px，整页 5280px。** 零横向溢出、零控制台错误。
点选某个分类后说明照常出现（正好 1 条，有验证）。

⚠️ **区块文案一并改了。** 「Selected Work / 3D Work Organized By Asset Type」
描述的是一个精选的个人作品集；这一区现在展示的是**所有创作者**的作品，
所以文案改成「最新作品 / 来自每一位创作者的三维资产」。

完成内容：

- `src/sections/Projects.jsx`：选「全部」时不再渲染六份说明
- `src/lib/i18n.js`：作品区的 kicker/title/intro × 3 语言
...
- `npm run test:unit`：281 通过
- `npm run test:api:db`：196 通过
- 线上实测（440px）：`gap` 1096→**330**、`total` 6046→**5280**、
  `overflow` 0、`summaries` 0（选「全部」时）
- VPS 部署：成功
- 接口验证：完整清单全过，含 301 与 `qrUrl` 检查（0）

备份路径：

- `/opt/mrright-portfolio.backup-20260909-045420`

待办事项：

- ⚠️ **首页 Hero 仍然是个人作品集的口吻**：「Hi, I am Right / Creating
  Next-Gen Props / 3D model and game art asset creator」。
  访客落在首页时，**没有任何一句话说明这是一个人人可发布的站点**。
  这是最后一处产品与门面不一致的地方，但改的是你的名字和标语，
  **需要你定**（见本轮对话末尾的建议）。
- 升级到平台收款需要：主体 + 持牌分账 + 每个创作者 KYC（见 ADR §6）
...
⚠️ **首页的作品区读的是 `/api/projects`——只有站主自己那四个。**
不管以后来多少创作者，首页都只会显示站主的作品。
现在改读市集目录（`/api/works`），并在目录为空时回落到内置项目——
全新安装或旧数据库不该看到一个空首页。
**作品对象带的是项目的字段名**（mapper 当初就是为这个保的），
所以那个区块不需要任何转换层，只把 `url` 映射成 `workUrl`——
它的链接本来读的就是这个名字。

⚠️ **没有任何地方告诉访客他也能发布**，而这正是 ADR §1 唯一在讲的事。
新增 `PublishCta`，放在首页和 `/explore`：
已登录直达作品面板，未登录直达登录页——**两边都是一次点击**，
而不是先送到一个「请先登录」的页面。

⚠️ **那个链接逼出一个真问题:账号页的标签是纯组件状态**，
所以 `/account/works` 会渲染成总览。
标签现在写进 URL，于是可以被链接、刷新也能停在原处。
用 replace 而不是 push——来回切标签不该把后退键填满。

完成内容：

- `src/components/PublishCta.jsx`（新增）
- `src/App.jsx`：首页取市集目录，加号召
- `src/pages/ExplorePage.jsx`：加号召
- `src/pages/AccountPage.jsx`：标签写进 URL
- `src/lib/i18n.js`：4 个键 × 3 语言
- `src/index.css`：号召样式
...
- `npm run test:unit`：281 通过
- `npm run test:api:db`：196 通过
- **浏览器端到端 8 项全过，用的是「不是站主」的创作者**：
  首页说明可以发布 → 未登录指向登录 →
  **首页显示别人的作品** → `/explore` 同样有号召 →
  已登录点击直接落在「作品」标签（不是总览）→ 作品列在那里 →
  刷新仍停在该标签 → 零控制台错误
- VPS 部署：成功
- 接口验证：完整清单全过，新增 `/account/works` 200；含 301 与 `qrUrl` 检查（0）

备份路径：

- `/opt/mrright-portfolio.backup-20260909-034749`

待办事项：

- 首页仍保留站主的 About / Experience / Contact 三个个人区块。
  产品已是市集，这些更像属于创作者主页而不是首页——
  **但那是删站主的个人内容，应该由他决定，我没有动。**
- Phase 4 剩余：界面外壳的视觉重构（纯视觉，不阻塞）
- 升级到平台收款需要：主体 + 持牌分账 + 每个创作者 KYC（见 ADR §6）
- ⚠️ **`@mrright` 的显示名仍然是 `111111`**
- 仍未决：外部 uptime 服务（需要你的账号）
...
## 下次从这里继续（截至 2026-09-09 第四十九轮收工）

### 2026-09-09（第四十九轮，当天第三轮）：补上「发布作品」的界面

⚠️ **发现了一个两天没人注意到的空缺：前端根本没有发布作品的入口。**
发布相关的 API 第三十八轮就写好了，但**客户端里从来没有任何代码调用过它**——
没有 `createWork`、没有资源上传、什么都没有。
站点上写着「人人可上传」，而第二个创作者只能自己手敲 API。

这比继续做视觉重构重要得多，所以先做了这个。

⚠️ **刻意做成草稿优先。**
一个「全部字段合法才肯保存」的表单，会把人在去找文件之前写的东西丢掉；
而创作者边攒素材边填，在这里是常态不是例外。
所以流程是：建草稿 → 存下已有的 → 文件到了再传 → 准备好了再提交。

⚠️ **价格按元输入、按整数分存储，只在一个地方换算一次。**
浮点数金额是这个项目不需要重新发现一遍的 bug 类别。

⚠️ **两件事由面板自己说，而不是让 API 用 400 去说**：
需要先有用户名（因为要去另一个页面改）、
定了价要先填收款方式（因为在第三个页面）。

⚠️ **列表端点返回的是摘要、没有 assets**，所以打开编辑器要单独取完整作品；
每次上传/删除都用 API 刚返回的那份替换，而不是重新拉一遍。

完成内容：

- `src/components/WorksPanel.jsx`（新增）
- `src/pages/AccountPage.jsx`：新增「作品」标签页
- `src/lib/api.js`：7 个方法（`getMyWorks` / `createWork` / `updateWork` /
  `setWorkStatus` / `deleteWork` / `uploadWorkAsset` / `deleteWorkAsset`）
- `src/lib/i18n.js`：34 个键 × 3 语言
- `src/index.css`：发布界面样式
...
- `npm run build` / `lint` / `test:openapi`：通过
- `npm run test:unit`：281 通过（i18n 用法测试又逼出一处：
  `worksUpload` 没被渲染——文件输入框本来就缺标签，补成真标签，
  顺带修了可访问性）
- `npm run test:api:db`：196 通过
- **浏览器端到端 9 项全过，用的是全新注册的账号**：
  没有用户名时提示去哪改 → 空态指向「新建作品」→ 建草稿并打开编辑器 →
  保存 → 上传封面并列出 → 提交审核变「审核中」→
  **价格存成 1250 整数分** → 管理员发布后出现在公开目录 → 零控制台错误
- VPS 部署：成功
- 接口验证：`CLAUDE.md` 第 9 条完整清单全过，含 301 与 `qrUrl` 检查（0）；
  线上四个作品仍在
...
- `/opt/mrright-portfolio.backup-20260909-033713`

**到这里，一个陌生人可以：注册 → 选用户名 → 发布作品 → 填收款方式 →
被人购买 → 确认到账 → 买家下载。整条链在浏览器里验证过。**

待办事项：

- 首页仍然是个人作品集的样子（Hero/About/Projects/Experience/Contact），
  而产品已经是市集——**没有任何地方告诉访客他也能发布**。
  这是下一个值得做的产品缺口，比视觉重构具体。
- Phase 4 剩余：界面外壳的视觉重构（纯视觉，不阻塞）
- 升级到平台收款需要：主体 + 持牌分账 + 每个创作者 KYC（见 ADR §6）
- ⚠️ **`@mrright` 的显示名仍然是 `111111`**
- 仍未决：外部 uptime 服务（需要你的账号）
...
## 下次从这里继续（截至 2026-09-09 第四十八轮收工）

### 2026-09-09（第四十八轮，当天第二轮）：给管理台字典补上「必须被渲染」的守卫

站点侧有这个守卫（第二十一轮起），这两天连着抓出两个真 bug；
**管理台一直是豁免的**——`i18n-usage.spec.js` 显式跳过 admin 路径。
现在补上了：`tests/unit/i18n-admin-usage.spec.js`。

**它当场抓出 5 个死键**，其中一个是我自己昨天加的：

| 键 | 处置 |
|---|---|
| `orders.settledBy` | **删**。它是给一个 `toOrder` 根本没返回的字段做的标签；凭证链已经用时间顺序更好地回答了「谁确认的」——所以删标签，而不是为了它去发明一个字段 |
| `common.save` / `common.done` | 删，早先遗留 |
| `galaxy.loading` / `galaxy.open` | 删，早先遗留 |

⚠️ **有两处必须做对，而且两处都是靠变异发现的、不是想出来的：**

1. **扫描范围是整个 `src/`，不是 admin 那两个目录。**
   `AdminTotpEnrolment.jsx` 在 `src/components/` 而不是 `src/components/admin/`，
   它那二十多个 `totp.*` 键第一版全被报成没用；
   而且**管理台把键当值传**（`deleteItem('entity.project')`、导航数据里的
   `labelKey`），只匹配 `t('...')` 调用形式会漏掉一大片。
2. ⚠️ **必须把字典自己的条目行从语料里剔掉。**
   不剔的话每个键都被自己的定义"提到"，检查就是装饰——
   我加了一个没人渲染的键进去，测试**照样绿**。
   第一次修法是**排除整个文件**，也是错的：`i18nAdmin.js` 里还有格式化函数，
   而那 14 个 `time.*` 键只有它们在用。
   现在只剔除形如 `'key': ...` 的行，并且**专门写了一条测试断言这个剔除生效**——
   那才是这个文件里承重的部分。

⚠️ **顺带记一个操作教训**：还原变异时我把 `git checkout --` 用在一个
**未跟踪的新文件**上，恢复不了；手工还原又留下了一行不可达代码，
lint 抓到了。新文件的变异要用备份文件还原，不能靠 git。

⚠️ **另一个**：提交信息里带反引号会被 shell 当成命令替换，吃掉了一句话。
长提交信息用 `git commit -F 文件` 传。

完成内容：

- `tests/unit/i18n-admin-usage.spec.js`（新增，4 条断言）
- `src/lib/admin/i18nAdmin.js`：删掉 5 个死键 × 3 语言 = 15 行
...
- `npm run build` / `lint` / `test:openapi`：通过
- `npm run test:unit`：**281 通过**（原 277，新增 4）
- `npm run test:api:db`：196 通过
- 变异验证两条：加一个没人渲染的键 → 「renders every key」失败；
  把字典条目放回语料 → 「does not count a key's own definition」失败
- VPS 部署：成功
- 接口验证：`CLAUDE.md` 第 9 条完整清单全过，含 301 与 `qrUrl` 检查（0）

备份路径：

- `/opt/mrright-portfolio.backup-20260909-032921`
...
## 下次从这里继续（截至 2026-09-09 第四十七轮收工）

### 2026-09-09（第四十七轮）：后台的凭证链视图

纠纷故事的最后一块。API 第四十五轮就有了，但管理员还得手动 curl；
现在管理台有「订单」这一节。

⚠️ **凭证链是这一屏的主体，不是藏在链接后面的细节。**
纠纷时它就是平台能拿出来的全部——平台没经手过这笔钱，也退不了。
「久未确认」列表排在最前，提示直说可做的动作是**联系或停用创作者，不是退款**。

⚠️ **这一节最值得做的一点：后台显示的收款方式取自订单快照，
不是创作者当前的设置。**
浏览器实测方式是：下单后、打开管理台前，让创作者把收款方式从支付宝改成微信——
界面显示的仍是买家当初看到的支付宝说明，**没有**显示改后的微信。
创作者事后改设置，抹不掉买家被告知过什么。

⚠️ **订单与久未确认列表在加载时软失败**（和 overview 一样）：
旧版服务端没有这些路由时，代价应该是少一个 section，不是整个管理台打不开。
凭证链**按需拉取**，不随列表一起——把每笔订单的完整经过都预先拉下来的管理台，
负载大半花在没人点开的记录上。

⚠️ **从截图里改掉一处**：显示名恰好等于角色名时，
凭证链渲染成「Buyer · Buyer」，看起来像渲染 bug。同名时只显示一次。

⚠️ **另外修掉一个我自己上一轮写坏的地方**：
`PROJECT_PROGRESS.md` 顶部的「下次从这里继续」标题重复了两次——
上一轮插入当日总览时字符串拼重了。已删掉重复的那行。

完成内容：

- `src/components/admin/AdminOrdersSection.jsx`（新增）
- `src/lib/admin/sections.js`：moderation 组新增 `orders`，且可搜索
- `src/components/admin/AdminIcon.jsx`：新增 orders 图标
- `src/lib/admin/i18nAdmin.js`：25 个键 × 3 语言
- `src/lib/api.js`：4 个管理端方法
- `src/Admin.jsx`：数据加载、`updateOrderStatus`、`loadOrderEvents`、section 渲染
- `src/index.css`：订单区样式
- `docs/OPERATIONS_DEPLOY.md`：纠纷时先去管理台，接口作为脚本化备选

commit：`9e7c4ff`（代码）、`9bd032a`（运维文档）

验证结果：

- `npm run build` / `lint` / `test:openapi`：通过
- `npm run test:unit`：277 通过（含管理台三语字典一致性）
- `npm run test:api:db`：196 通过
- **浏览器端到端 8 项全过**：导航出现 → 订单列出 → 买家声明在行内 →
  凭证链显示「买家声称已付款」与「下单」→
  **显示买家当初看到的支付宝、不显示创作者改后的微信** → 零控制台错误
- VPS 部署：成功
- 接口验证：`CLAUDE.md` 第 9 条完整清单全过，含 301 与 `qrUrl` 泄露检查（0）
- 三个新管理端点未授权时 401

备份路径：

- `/opt/mrright-portfolio.backup-20260909-031711`

**至此 ADR 七个阶段的功能面全部落地**（Phase 7 是「创作者直收」版本，
平台收款需要主体，见 ADR §6）。

待办事项：

- Phase 4 剩余：界面外壳的视觉重构（纯视觉，不阻塞）
- ⚠️ **管理台字典没有「每个键都被渲染」的检查** ——
  站点侧有（`i18n-usage.spec.js`），但它显式跳过 admin 路径。
  站点侧那个测试这两天连着抓出两个真 bug，管理台缺同样的守卫。
- 升级到平台收款需要：主体 + 持牌分账 + 每个创作者 KYC（见 ADR §6）
- ⚠️ **`@mrright` 的显示名仍然是 `111111`**
- 仍未决：外部 uptime 服务（需要你的账号）
...
### 今天三轮的总览（2026-09-08，第四十四~四十六轮）

**主题做完了，支付换了一条路并做完了。** 昨天七轮把站点变成平台，
今天三轮把最后两块补上。

| 轮次 | 做了什么 | 线上可见 |
|---|---|---|
| 44 | Phase 6：个人主题（强调色 + 背景 + 字体） | ✅ |
| 45 | 支付模型改为「创作者直收」+ 凭证链（服务端） | 端点就位 |
| 46 | 购买流程前端 + 创作者收款/确认界面 | ✅ **可以卖了** |

**现在能做的事**：在 `/account` → 交易里填收款方式（支付宝/微信，最多 4 个），
给作品定价，买家就能下单、付款、由你确认到账、然后下载。

⚠️ **支付这条路是当天推翻重来的，理由记在 ADR §6：**
你问「中国地区怎么接支付」，查下来三个事实把 Stripe Connect 否掉了——
Stripe 不支持大陆作商户国家；大陆直连要 ICP 备案而备案要求服务器在大陆
（这台在日本）；平台代收代付需要支付牌照。
中途做了「平台收款+人工确认」，你看过之后选了「创作者各自收款，以后升级」。

⚠️ **把「公平」和「凭证」拆开，是这件事能落地的关键判断。**
强制退款必须握着钱（要主体、持牌分账、每个创作者 KYC，没有替代路径）；
但**留证据完全不需要这些**，而且是以后升级时同样需要的那套证据。
所以先做证据，钱的杠杆等有主体再说。

**今天学到的三件事，值得带走：**

1. ⚠️ **变异测试必须保证被测代码仍然有效。** 今天有**三次**变异
   把 SQL 查询本身改崩了（参数数量/类型不匹配），那种失败只证明代码被改坏，
   不证明规则有用。保持查询有效、只废掉规则，才是真正的证明。
2. ⚠️ **一条"覆盖了某规则"的测试，可能根本没覆盖到。**
   抽成归零的守卫是死代码（路由不传 fee），删掉它 194 条全过。
   提成 `platformFeeBasisPoints()` 具名函数并单独写单元测试后才咬得住。
3. ⚠️ **i18n 用法测试连着抓出两个真 bug。**
   它只是报告「这个键没有任何东西渲染」，但背后是
   `creatorSales` 藏掉了所有待确认订单（等于什么都卖不出去）、
   以及买家无法取消订单（会在创作者列表里挂到天荒地老）。

**测试规模**：单元 245 → **277**，API-DB 契约 162 → **196**。

---

### 2026-09-08（第四十六轮）：购买流程的前端

买家侧和创作者侧都做完了,整条路在真实浏览器里跑通。

⚠️ **风险提示在收款码之前,而且它把收款码挡住了。**
不点掉那段提示,收款方式根本不显示。
要转钱给陌生人的人应该**先**知道自己的保障是什么,
而「链接到条款」不算告知。提示原文说明:平台不经手这笔钱、
因此无法代为退款;个人转账没有拒付通道;
保障是「平台会封禁不发货的创作者并保留完整订单记录」。

⚠️ **接前端时发现两个真 bug,都是 i18n 用法测试逼出来的**
（它报了两个「定义了但没有任何东西渲染」的键):

1. **`creatorSales` 只查 `status = 'paid'`,待确认订单永远到不了创作者手里。**
   而创作者确认到账**就是**这个模型的支付系统——那个确认按钮永远不会渲染,
   等于什么都卖不出去。改成查全部、待确认排最前;
   金额合计仍然只算已付的,**没人确认的订单不是收入**。
   变异回原样 → 新加的测试立刻失败。

2. **买家下了单没有任何办法取消。** 点了购买又改主意,
   那笔单会永远挂在创作者列表里,并出现在管理员的「久未确认」列表中,
   看起来像创作者在无视一笔根本没发生的付款。
   新增 `PATCH /api/orders/:id/cancel`(只能取消自己的、只在待确认时),
   取消这个动作本身也进凭证链。

完成内容：

- `src/components/WorkPurchase.jsx`（新增）：购买状态机
  （未登录／免费／已拥有／创作者未开售／下单／付款／等待确认／取消）
- `src/components/SellerPanel.jsx`（新增）：收款方式设置、销售列表（含确认到账）、购买记录
- `src/pages/AccountPage.jsx`：新增「交易」标签页
- `src/lib/api.js`：10 个新方法
- `src/lib/i18n.js`：35 个键 × 3 语言
- `server/index.js`：`PATCH /api/orders/:id/cancel`
- `server/postgres/ordersStore.js`：`creatorSales` 修复
...
- `npm run build` / `lint` / `test:openapi`：通过
- `npm run test:unit`：277 通过
- `npm run test:api:db`：**196 通过**（原 194，新增 2）
- 变异验证：`creatorSales` 改回只查 paid → 新测试失败
- **浏览器端到端**（一次性数据库 + 真实服务）9 项全过：
  购买按钮→风险提示先于收款码→提示未点掉时收款码不显示→
  点掉后显示创作者的说明→买家填付款备注→
  **未确认时仍然拿不到下载票据**→创作者看到待确认订单→确认后买家显示「已拥有」→
  零控制台错误
- VPS 部署：成功
- 接口验证：`CLAUDE.md` 第 9 条完整清单 + 301
- 线上抽查：公开响应有 `acceptsPayment`、**没有** `qrUrl` / `methods`；
  未登录取消订单 401

备份路径：

- `/opt/mrright-portfolio.backup-20260908-062325`

**创作者直收这条路现在是完整可用的。** 你在 `/account` → 交易 里
填上支付宝/微信收款方式，给作品定个价，就能卖了。

待办事项：

- 后台的凭证链视图（API 有了：`/api/admin/orders/:id/events`、
  `/api/admin/orders/stale`；管理台界面还没接）
...
每个登录用户可以设置自己的主题——你最早点名要的功能之一。三个旋钮：
强调色 + 背景预设 + 字体预设。只作用于**他自己的作品页和创作者主页**。

⚠️ **刻意不做自由 CSS。** 创作者的样式表可以重绘他页面周围的平台外壳、
盖住举报按钮、或者仿造一个结算弹窗——自由样式表买到的东西不值这些风险。

⚠️ **token 用 CSSOM 写入（`element.style.setProperty`），不注入 `<style>`。**
本站的 CSP 检查的是**被解析的 style 元素和 style 属性**，
对象模型的属性写入不在管辖内（第 25 轮就是搞清楚这条的）。
所以不需要 nonce、不需要改策略。

⚠️ **主题挂在页面元素上，不挂 `:root`，并在卸载时移除。**
否则它会跟着访客走到 `/explore`——那种 bug 要过两次导航才暴露。
浏览器实测验过：离开作品页后 root 上的 token 是空的。

⚠️ **强调色必须相对背景达到 3:1**（WCAG 对 UI 组件的下限）。
否则创作者会发布一个自己的链接都看不见的页面，而且不知道为什么。
拒绝时把**实测比值**一起返回，不然「换个颜色」等于让人瞎猜。

⚠️ **`--theme-on-accent` 用纯黑/纯白，取对比度更高的那个——这里发现了一个真问题。**
最初用的是站点的近黑色 `#0b0b10`，全色域扫描发现 **62 个本来合法的强调色
掉到 4.5:1 以下**（最低 4.46）。换成纯黑后，
「黑白取优」的最坏情况恰好在两者相等处，即 `(L+0.05)² = 1.05 × 0.05`，
算出来 **4.58:1**——高于 WCAG 的 4.5。所以**任何**强调色都可读，
这是构造保证而不是巧合。测试同时扫了整个色立方并断言了那个代数结果。

⚠️ **有一条测试是我自己写错的，失败暴露的是测试而不是代码。**
我拿 `#1e3a8a`（深蓝）当「深色强调色」的例子，但它在 ink 底上只有 1.78:1，
**本来就会被校验拒绝**，于是回落到默认色。改成断言性质（「总是选对比更高的那个」）
并在合法色上扫描，比硬编码颜色强得多。

完成内容：

- `server/themes.js`（新增）：预设、校验、对比度、token 解析
- `authStore.setTheme`、`getSessionUser` 补选 `theme` 列
  （不补的话 `GET /api/account/theme` 永远返回默认值）
- 3 个端点：`GET /api/theme-options`、`GET/PUT /api/account/theme`
- 主题随公开 profile 和作品的 creator 一起下发（`theme` 三旋钮 +
  `themeTokens` 已解析）——**预设到颜色的解析留在服务端**，
  客户端不需要知道 `ink` 是什么颜色
- `src/lib/theme.js`：CSSOM 写入，只接受 `--theme-*` 前缀
- `src/components/ThemeEditor.jsx`：编辑器 + 实时预览，
  选项来自服务端，**预览用的是和真实页面同一套 token**
- `src/pages/WorkDetailPage.jsx` / `PublicProfilePage.jsx`：穿上主题
- `src/lib/i18n.js`：13 个键 × 3 语言

修改文件：见上；另有 `src/index.css`、`src/pages/AccountPage.jsx`、
`server/postgres/mappers.js`、`tests/unit/themes.spec.js`（新增）、
...
- `npm run build` / `lint` / `test:openapi`：通过
- `npm run test:unit`：**265 通过**（原 245，新增 20）
- `npm run test:api:db`：**169 通过**（原 162，新增 7）
- `verify-schema-migration`：通过
- 变异验证：关掉对比度校验 → 契约测试失败；
  信任数据库存储值不再校验 → 单元与契约**各自独立**失败
- VPS 部署：成功
- 接口验证：`CLAUDE.md` 第 9 条**完整清单**跑过，含
  `/projects/md-leimu` **301**
- 线上主题端点：4 个背景、3 个字体、默认值正确；
  作品的 creator 带解析好的 4 个 token
- 浏览器实测：强调色 `#ff9966` 落到页面上、serif 标题生效、
  **离开作品页后主题没有外泄**、零控制台错误

备份路径：

- `/opt/mrright-portfolio.backup-20260908-045503`

待办事项：

- Phase 7 支付（Stripe Connect、结算、权益、付费下载）—— 最后一块
- Phase 4 剩余：界面外壳的视觉重构（不阻塞）
- ⚠️ **`@mrright` 的显示名仍然是 `111111`**
- `API_V1_FREEZE_PLAN.md` §7 的错误码数字已过期（26 vs 39）
- 仍未决：外部 uptime 服务（需要你的账号）
...
## （前一日）下次从这里继续（截至 2026-09-07 第四十三轮收工）

### 一天七轮的总览（2026-09-07，第三十七~四十三轮）

**站点从「个人 3D 作品集」变成了「人人可上传的作品平台」。** 这是一天之内
完成的，下面是全貌；每一轮的细节在后面各自的小节里。

| 轮次 | 做了什么 | 线上可见 |
|---|---|---|
| 37 | Phase 1：作者与作品的数据模型，7 张表 + 4 个列，迁移 4 个作品 | 无（故意） |
| 38 | Phase 2：发布生命周期（草稿→审核→发布）9 个端点 | 无（故意） |
| 39 | Phase 2 完：多文件上传，配额堵洞 | 无（故意） |
| 40 | Phase 3：`/explore` 与 `/w/:handle/:slug` 两个新页面 + SEO | ✅ 前端首次改动 |
| 41 | Phase 3 完：创作者主页的作品标签页 | ✅ |
| 42 | Phase 5：讨论区（单层嵌套、排序、点赞、置顶） | ✅ |
| 43 | Phase 4 核心：`/projects/:slug` **301** 到新地址 + 作品点赞 | ✅ 老地址已交棒 |

**现在的线上形态：**

- `/explore` 作品目录，可搜索、按分类筛选、分页
- `/w/mrright/:slug` 单个作品：3D 查看器、规格、文件列表、点赞、讨论区
- `/u/mrright` 创作者主页，带作品标签页
- `/projects/:slug` **301** 到对应的 `/w/...`（四条全部生效）
- 数据：4 个作品、8 条资源（24.7MB）、2 个赞、2 条评论，全部迁移自旧结构

**四条贯穿全天的判断，值得记住：**

1. ⚠️ **「不用全站 3D」这个改口保住了前面几轮的成果。** 第 26–28 轮花三轮把
   three.js 移出关键路径，第 30 轮才让 3D 星图能用键盘。全站 3D 会全部还回去。
   ADR §5 把边界写死了。
2. ⚠️ **不要照抄 `content.js`。** 一个项目 = `content.js` 合并 `project_overrides`
   加 `custom_projects` 减 `deleted_projects`。迁移只能走 `projectStore.listProjects`。
3. ⚠️ **动手前查生产数据。** `download_requests`/`tickets`/`events` 全是 0——
   下载流程从来没人用过，所以 301 丢掉它不花代价；`project_likes` 和
   `project_comments` 各有 2 行，所以必须迁。**这条数据把一个产品决策直接做掉了。**
4. ⚠️ **变异测试要保证被测代码仍然有效。** 验证评论状态过滤时，前两次变异
   把 SQL 查询本身弄坏了（参数数量/类型不匹配）——那种失败证明不了规则有用，
   只证明代码被改崩了。第三次保持查询有效、只废掉规则，才是真正的证明。

**今天新增的工具（都在 `scripts/`）：**

- `verify-schema-migration.mjs` — 把一次性库从旧 schema 升到新 schema 再断言
  （此前只测过在空库上建表，而生产只走 ALTER 那一半）
- `verify-works-migration.mjs` — 用**真实脚本子进程**在一次性库上演练迁移
- `creator-account.mjs` — 建/启用创作者（密码走 echo-off，不进 argv）
- `migrate-works.mjs` — 复制（非移动）项目为作品，含文件大小与互动迁移
- `lib/disposable-postgres.mjs` — 一次性 PostgreSQL 集群，两个校验脚本共用

**测试规模变化：** 单元 242 → **245**，API-DB 契约 **93 → 162**。

---


### 2026-09-07（第四十三轮，当天第七轮）：301 上线 —— 老地址交棒给新地址

`/projects/:slug` 现在 **301** 到 `/w/mrright/:slug`，四条全部生效、目标全部能打开。

⚠️ **上一轮我说"要么补齐点赞和下载、要么接受缺失"，先去查了生产数据，
结论把决定做掉了一半：**

| 表 | 行数 | 结论 |
|---|---|---|
| `download_requests` | **0** | 下载申请流程**从来没有人用过** |
| `download_tickets` | **0** | 同上 |
| `download_events` | **0** | 同上 |
| `project_likes` | **2** | 真实内容，必须迁 |
| `project_comments` | **2** | 真实内容，必须迁 |

所以：**下载不补**——重定向丢掉一个零使用的功能不花任何代价，
而在 Phase 7 的权益模型出来之前重建它，等于要建两遍。
**点赞和评论补齐并迁移**——它们不是零。

⚠️ **301 是有条件的，这是它可回滚的原因。**
只有当存在一个**已发布**、且 `source_slug` 等于这个 slug 的作品时才重定向；
其他情况照常提供老页面。**关于重定向本身没有存任何东西**，
所以把作品改成 hidden，老地址就回来了——这条写成了测试。
变异验证：把「已发布」这个条件放宽 → 立刻失败（那会把永久重定向指向一个 404）。

⚠️ **内部链接直接指向终点，不走重定向。**
`/api/projects` 现在带 `workUrl`，首页据此链接；sitemap 在一个项目有了作品后
就不再列它的老地址——**给爬虫一个会 301 的地址，等于让它做两遍功、
并对同一个页面说两件事**。

⚠️ **点赞复用了项目点赞那套签名 cookie，但有一处不同:读取计数的 GET 不发 cookie。**
给每个爬遍目录的爬虫都发一个身份,而答案本来就一样——读操作不该干这事。
这条也有测试。

⚠️ **作品页补上了 ProjectDetail 有而它没有的规格块**（格式/大小/制作工具/
查看器特性）。这些字段本来就在 work 对象上——mapper 当初保了项目的字段名——
所以之前的缺失纯粹是漏了。

⚠️ **线上量出真实缺陷:文件列表把整页横向撑出去 12px**
（440px 视口下 `scrollWidth=452`）。真正撑出去的是那个 `<a>`——
**不明确允许,它就不会缩到比文字更窄**。文件名在 34rem 以下换到自己一行，
360/440/768 三个宽度重新量过，都等于视口宽度。

完成内容：

- `worksStore`：`getWorkLikes` / `toggleWorkLike` / `findWorkBySourceSlug` /
  `mapSourceSlugsToUrls`
- 3 个端点：读点赞、切换点赞、`GET /projects/:slug` 的 301
- `readLikeIdentity`（只读身份，不发 cookie）
- `scripts/migrate-works.mjs`：迁移 `project_likes` 与**已发布的**
  `project_comments`（pending 的不迁——`work_comments` 没有待审状态，
  迁过去等于把管理员拦下的内容发布出去）
- `src/pages/WorkDetailPage.jsx`：点赞按钮 + 规格块
- `src/sections/Projects.jsx`：链接指向 `workUrl`
- `src/index.css`：文件列表换行修复

commit：`884d638`、文件列表修复

验证结果：

- `npm run build` / `lint` / `test:unit`(245) / `test:openapi`：全通过
- `npm run test:api:db`：**162 通过**（原 152，新增 10）
- `verify-works-migration`：全绿（含点赞/评论迁移与幂等）
- 变异验证：301 不再要求目标已发布 → 两条测试失败
- VPS 部署：成功（当天第八、九次）
- **线上 301 实测**：四条全部 301，目标全部 200
- **浏览器实测**：从老地址出发，落在 `/w/mrright/fire-extinguisher-next-gen`，
  零控制台错误
- 生产迁移：`work_likes=2`、`work_comments=2`，
  源表 `project_likes=2`、`project_comments=2` 原样保留（是复制，可回滚）
- 迁移落点正确：`fire-extinguisher-next-gen` 有 1 赞 2 评论，
  `md-leimu` 有 1 赞
- 读点赞不发 `set-cookie`、sitemap 里 `/projects/` 出现 0 次

备份路径：

- `/opt/mrright-portfolio.backup-20260907-163215`

**Phase 4 的核心（301）完成。** 剩下的「新界面外壳」是纯视觉重构，
可以随时做，不阻塞任何东西。

待办事项：

- Phase 6 主题（每个登录用户自定义自己的主题）—— **下一步**
- Phase 7 支付（含源码包的付费下载路径，会一并把下载重建成购买后发票据）
- Phase 4 剩余：界面外壳的视觉重构（不阻塞）
- ⚠️ **`@mrright` 的显示名仍然是 `111111`**，现在出现在作品页、卡片、
  创作者主页和每条评论旁
- `API_V1_FREEZE_PLAN.md` §7 的错误码数字已过期（26 vs 39）
- 仍未决：外部 uptime 服务（需要你的账号）


### 2026-09-07（第四十二轮，当天第六轮）：作品的讨论区（Phase 5 提前）

⚠️ **本轮原计划是 Phase 4 的 `/projects/:slug` → `/w/:handle/:slug` 301，
做之前发现它还不能做。**
`/projects/:slug` 那个浮层有评论、点赞、下载申请；`/w/:handle/:slug` 当时只有
简介和文件列表。**把丰富的页面 301 到更薄的页面是降级**，而 301 是永久的、
浏览器会长期缓存。所以先把作品页补齐，再做重定向——ADR 的阶段顺序不是圣旨，
出现真实依赖时应该让路。

于是做了 Phase 5：讨论区。单层嵌套、可按「最热/最新」排序、可点赞、
作者可置顶——就是你要的 YouTube 那个形状。

⚠️ **发评论必须登录，所以这里没有审核队列。**
`project_comments` 有队列，是因为匿名评论会立刻发布，而唯一的防线是一个
在这台机器上根本不成立的 per-IP 限流（`docs/OPERATIONS_CLIENT_IP.md`）。
**一个账号比一个要人去读的队列是更好的答案。**

⚠️ **两处刻意的不对称：**
1. **回复的回复会被压平到同一个父级。** "单层"才是人们期待的形状，
   任意深度嵌套是另一种、更糟的 UI，没人要过。
2. **作品作者「隐藏」别人的评论，评论作者「删除」自己的。**
   对别人的话语，可逆的管理才是恰当的权力；不可逆的不是。
   被隐藏的评论对公众消失，但作品作者仍能看到并标记为「已隐藏」，
   所以撤销是可见可行的。

⚠️ **状态过滤写在 SQL 里**，调用方忘了也漏不出去。
变异验证时前两次尝试都把查询本身弄坏了（参数数量/类型不匹配），
**那种失败证明不了过滤规则有用**；第三次用 `(true OR $3::boolean OR ...)`
保持查询有效、只废掉规则，才精确失败在"隐藏的评论对公众不可见"那条断言上。

⚠️ **合同测试覆盖了 API，但 React 组件没有被真正点过。**
用一次性数据库 + 真实浏览器跑了一遍：发帖、点赞、回复、置顶，
8 项检查全过、零控制台错误。

⚠️ 截图里第一条评论的文字看着偏蓝、回复是白的。**量了一下,两段都是纯白**
——是 Playwright `fill()` 残留的选中高亮,不是 bug。查了没白查。

完成内容：

- `server/postgres/worksStore.js`：7 个评论方法
- `server/postgres/mappers.js`：`toWorkComment`
- 6 个端点：列表 / 发表 / 删除 / 改状态 / 置顶 / 点赞
- `src/components/WorkComments.jsx`（新增）
- `src/lib/api.js`：6 个客户端方法
- `src/lib/i18n.js`：20 个键 × 3 语言
- `src/index.css`：讨论区样式
...
- `npm run build`：通过
- `npm run lint`：通过
- `npm run test:unit`：245 通过
- `npm run test:api:db`：**152 通过**（原 139，新增 13）
- 变异验证：状态过滤失效 → "对公众不可见"失败；
  去掉跨作品父级检查 → 两条回复测试失败
- 真实浏览器端到端：8 项全过
- VPS 部署：成功（当天第七次）
- 接口验证：九个接口全部 200
- 评论端点线上实测：匿名可读 200、匿名发帖 401、
  不存在的作品 404、未登录点赞 401

备份路径：

- `/opt/mrright-portfolio.backup-20260907-155949`

待办事项：

- **Phase 4 的 301 现在可以做了吗？还不完全**：作品页仍缺
  点赞和下载申请（`/projects/:slug` 有）。要么补齐，
  要么接受这两项在新页面上暂缺——**这是个需要你拍板的取舍**
- Phase 4 新界面外壳
- Phase 6 主题、Phase 7 支付
- ⚠️ **`@mrright` 的显示名仍然是 `111111`**
- `API_V1_FREEZE_PLAN.md` §7 的错误码数字已过期（26 vs 39）
- 仍未决：外部 uptime 服务（需要你的账号）
...
| 决策项 | 你的指示 | 结果 |
|---|---|---|
| 第 4 条 中文文案串项目 | 中文与英文对应 | ✅ 已改，线上四个项目中文标题不再重复 |
| 第 3 条 231.8MB 孤儿文件 | 删除 | ✅ 已归档后删除，释放 232MB |
| 第 5 条 17 个未渲染 i18n key | 删除 | ✅ 51 条字符串已删（`b8212af`，已部署） |
| 第 2 条 机场子域过期证书 | 废弃证书，不用管 | ✅ 未动，已在清单里标注为「已决定不处理」 |

⚠️ **第 4 条实际比之前报告的更严重：串的是三栏，不是两栏。**
`titleZh`、`summaryZh` 之外，**`workflowZh` 也是祭坛那款的工艺描述**
（"石材块面、红色玻璃、蓝色烛光"）。三栏一起修了，改成与
`title` / `summary` / `workflow` 基础栏一致的长剑文案。
⚠️ **改动走的是 `/api/admin/projects/:slug` PATCH，不是手写 SQL。**
那个端点用**整份 payload 覆盖**，缺字段会被清空——所以带上了全部现有字段，
只替换三个中文栏。改完逐项核对过 image / modelUrl / stack / isPublic 都没变，
祭坛那个项目也未被波及。
⚠️ **还剩一处没改**：`formatZh` 的值是英文 `"GLB model"`，
`format` 基础栏是 `"Hand-painted scene"`——这不属于"串项目"，是"没翻译"，
不在你这次的指示范围里，留着没动。

⚠️ **第 3 条是删生产上传文件，而 `CLAUDE.md` 第 3 条明写「不要删除上传文件」。**
你是所有者可以推翻自己的规则，所以执行了——但**用的是可逆方式**：
1. 先重跑检测拿当前名单（两天前的数字不能直接用）
2. 打包到 `/root/orphaned-uploads-20260906.tar.gz`（223M，**在 `/opt` 之外**，
   不与硬链接备份纠缠）
3. **先验归档再删**：解开那个被点名的源文件，比对 sha256 一致，才动手
4. 按名单逐个删，不用通配符扫目录
5. 删后重跑检测：孤儿 0，剩余 7 个文件正好是四个项目在用的图与模型，
   五个线上资源 URL 全部 200

⚠️ **那个源文件已在归档里，随时能取回**：
`1781017698552-tl-miehuoqi.glb`（42,395,752 字节）——
`server/content.js` 的注释点名它是次世代灭火器模型
（`/models/fire-extinguisher-4k.3fa834b2.glb`）的源文件。
要取回：`tar -xzf /root/orphaned-uploads-20260906.tar.gz -C /opt/mrright-portfolio/public ./uploads/models/1781017698552-tl-miehuoqi.glb`
⚠️ **归档本身没有异地副本**，和数据库备份一样在同一块磁盘上
（你 2026-08-12 明确说过不需要异地备份，这里沿用同一决定）。

⚠️ **第 5 条删完之后，那道闸反而更严了。**
`tests/unit/i18n-usage.spec.js` 的清单现在是**空数组**，
断言从"没有新增的未渲染 key"变成"字典里一个未渲染的 key 都不该有"。
做了变异验证：加回一个没人渲染的 key，测试立刻红并点名它。
⚠️ 同时**删掉了那条配套断言**（"每个钉住的 key 在三语里都有定义"）——
对空清单它什么都不测，而"通过但什么都没验"正是这个仓库栽过跟头的地方。
...
|---|---|
| `npm run lint` | 通过 |
| `npm run test:unit` | **242 通过 / 11 个文件**（原 243，减 1 是删掉的那条空转断言） |
| `npm run build` | 通过（未重定向输出） |
| `npm run test:api` | 60 通过 / 13 skipped |
| 变异验证（i18n 闸门） | 加回一个未渲染 key → 立刻变红并点名 `zzTestUnusedKey`；还原后 4/4 通过，文件 diff 确认完全还原 |
| 归档校验（删文件前） | 解出源文件比对 sha256 **一致**，42,395,752 字节 |
| 部署前备份 | `/opt/mrright-portfolio.backup-20260906-063349` |
| VPS 部署 | 成功（`b8212af`，06:33 UTC），服务重启成功 |
| 线上 `npm run check:production` | **18 项全过**，0 warn 0 fail |
| 线上七条路由 | 全部 200（健康检查已覆盖） |
| 线上在用资源 URL | 五个图/模型 URL 全部 200 |
| 删除后复检 | `find-orphaned-uploads` 报 **孤儿 0**，剩余 7 个文件 |
| 部署后 uploads 复查 | 仍是 7 个文件 / 20MB，**未被构建产物灌回** |
| 项目数据复核 | 四个项目中文标题无重复；祭坛项目未被波及 |

### 改动的文件（第三十六轮）

- `src/lib/i18n.js`（删 53 行 = 51 个条目，其中 2 个条目的值是跨行写的）
- `tests/unit/i18n-usage.spec.js`（清单清空、注释改写、移除已空转的断言）
- **生产数据**：`project_overrides` 中 `crimson-rune-greatsword` 一行的
  `titleZh` / `summaryZh` / `workflowZh`（经 admin API，非手写 SQL）
- **生产文件**：`public/uploads/` 下 24 个孤儿文件已删（先归档）

**2026-09-06 收工（这一天做了三轮：第三十四、三十五、三十六轮）：**

- 提交：`bc7b2bc`、`bb19828`、`31d6485`（三十四轮）、`e6b304e`、`b3962cd`（三十五轮）、
  `b8212af`（三十六轮）+ 本次文档提交，**全部已 push**
- 线上是 `b8212af`（06:33 UTC 部署）
- **部署后复查 uploads 仍是 7 个文件 / 20MB**，没有被构建产物灌回
  （第十六轮那道闸仍然有效）
- 线上 18 项健康检查全过；本机无服务在跑
- **数据库写操作：一次**，`project_overrides` 的一行 UPDATE（第 4 条），
  经 admin API，无 DELETE / DROP
- 未完项仍是 7 条；**「待你决策」5 → 1 条**

---

### 2026-09-06（第三十五轮，当天第二轮）：让检查器真的有人跑


上一轮留了一条「待你决策」：健康检查做好了但没人定时跑，三条路怎么选。
你回「由你决定」，所以我选了、也做了 —— **方案 1：VPS cron + 失败时发信**。

**为什么选它，以及为什么不选另外两条：**

- **方案 2（外部 uptime 服务）在"整机挂了"这一项上是唯一有效的**，
  但它需要注册第三方账号——那是你的账号、你的收件箱，不该由我替你开。
  **它和方案 1 不是二选一，是互补**，我把它留在「待你决策」里当作建议。
- **方案 1 我现在就能做完，全部在你已有的基础设施上**，而且
  **9 月 4 号那次它是会响的**——当时机器好好的，只是证书过期，cron 照跑不误。
- 方案 3（保持手动）等于维持现状，而现状恰恰是出事的原因。

| | 第三十五轮 |
|---|---|
| 做的事 | `scripts/notify-production-health.mjs` + VPS cron（每天 12:00 UTC） |
| 部署 | ✅ `e6b304e`，06:01 UTC |
| 数据库 | 无变更，本轮没有跑过任何数据库写语句 |
| 回滚点 | `/opt/mrright-portfolio.backup-20260906-060144` |

⚠️ **通知器和检查器是两个脚本，这是刻意的。**
`check-production.mjs` 严格只读（只发 GET，无任何副作用），这个性质值得能够
毫无保留地讲出来。发信是副作用，所以放在 `notify-production-health.mjs` 里，
并且是**以子进程方式调用**检查器、只吃它的 `--json` 输出。

⚠️ **成功时静默，是有代价的选择。** 每天一封"一切正常"是你很快就不会再读的信，
而一个你已经训练自己忽略的告警不是告警。**代价是：cron 要是停了，
看起来和网站健康一模一样。** 这个盲区我没法用更多代码解决——
它正是方案 2 的用武之地。

⚠️ **两种本来会静默的情况，被特意做成会响：**
检查器**自己**跑不起来时照样发信；失败了但 SMTP 没配置时**退出码非 0 并明说没人被通知**，
而不是假装已告警。

⚠️ **cron 环境验证过了，不是只"手动跑通"。**
用 `env -i HOME=/root PATH=/usr/bin:/bin` 模拟 cron 的极简环境实跑通过；
`node` 的真实路径确认是 `/usr/bin/node`（wrapper 里写的绝对路径）；
极简环境下 `isEmailDeliveryConfigured()` 也确认为 true。

⚠️ **wrapper 放在 `/usr/local/bin/mrright-health-check.sh`，不在 `/opt` 里。**
`/opt/mrright-portfolio` 每次部署整体替换，而且它的备份是硬链接的——
放进去要么下次发版丢掉，要么和备份纠缠。

⚠️ **真实发信验证过一次**（对着 `expired.badssl.com` 触发，主题里带被检查的域名，
所以你能一眼认出是测试）。SMTP 事务成功。
**但按老规矩，发信成败不能只看返回值** —— 收件箱那一步要你自己确认。
如果没收到，先查 Resend 后台再怀疑脚本。

### ⚠️ 顺手发现：机场那套域名有两张证书已经过期两个多月

装 cron 时看 crontab，发现这台机器上**有两套证书工具并存**，且是分工不是冲突：

| 工具 | 管什么 | 状态 |
|---|---|---|
| certbot（systemd timer） | `mrright.blog` / `www` —— **nginx 网站 vhost 加载的正是这份** | 9/4 修好，有效到 12-03 |
| acme.sh（cron 10:55） | `hy.` / `jp.` / `suijp.mrright.blog`、`mrmyself.life` | **两张已过期** |

```text
hy.mrright.blog      Jun 29 09:20:35 2026 GMT   !! 已过期
suijp.mrright.blog   Jul 28 09:56:36 2026 GMT   !! 已过期
jp.mrright.blog      Oct 28 13:32:46 2026 GMT   有效
mrmyself.life        找不到证书文件
```

⚠️ **我没有动它，是刻意的**：这些是机场节点那套域名，不是网站。
你 2026-08-11 明确定过「不动机场基础设施」，而续期会触发验证流程、
可能影响正在用节点的人。**这属于你的决定**，已记进「待你决策」。
⚠️ 另外注意：**本轮做的健康检查只看 `mrright.blog`，不看这些子域**——
它们不在网站的职责范围内，硬塞进去会让"网站健康"这个信号变得含糊。

**2026-09-06 当天收工（这一天做了两轮：第三十四轮和第三十五轮）：**

- 提交：`bc7b2bc`、`bb19828`、`31d6485`（第三十四轮）、
  `e6b304e`（第三十五轮）+ 本次文档提交，**全部已 push**
- 线上是 `e6b304e`（06:01 UTC 部署）
- 本机无服务在跑；**本轮没有跑过任何数据库写语句**
- 未完项仍是 7 条；「待你决策」4 → **5 条**
  （新增机场子域证书；原「健康检查没人跑」那条**已由本轮关闭**）

---

### 2026-09-06（第三十四轮，当天第一轮）：CSP 收紧落地 + 给"不会报错的故障"装一个探子


两条轨并行。**一条是上一轮调研的落地，一条是上一轮事故的后续。**

| | 轨 A | 轨 B |
|---|---|---|
| 做的事 | `style-src` 去掉 `'unsafe-inline'` 并上线 | `npm run check:production` 生产健康检查 |
| 对应清单 | 路线图第 9 条 ✅ **关闭** | 2026-09-04 证书事故的后续 |
| 部署 | ✅ `bb19828`，05:43 UTC | 不影响运行时，随同一次部署带上去 |
| 数据库 | 无变更，本轮没有跑过任何数据库写语句 | 同左 |
| 回滚点 | `/opt/mrright-portfolio.backup-20260906-054258` | 同左 |

线上现在的 header：`style-src 'self' https://fonts.googleapis.com`。

⚠️ **上一轮 ADR 第 7 节的三个门，关了两个，第三个诚实地留着。**
- **门 1（登录态后台）**：原以为需要生产凭证，其实不需要——
  `admin-console.spec.js` 用 fixture 就能渲染完整登录态控制台，含 `.admin-galaxy`
  那个 3D canvas 和 `--dot-size` 自定义属性。六个检查点，违规全 0，
  其中一个检查点有 **144 个元素带着 inline style 属性**、`<style>` 数为 0。
- **门 2（有内容的社区列表）**：`site-routing.spec.js` 的 posts fixture，三个检查点全 0。
- **门 3（非 Chromium 引擎）**：**没测，也没装浏览器**。ADR 里把推理收紧了
  （CSP 的检查定义在"被解析的内容"上，CSSOM 属性写入在任何引擎都不触发；
  Safari 反而更安全——它两个子指令都不实现，统一回落到 `style-src`），
  但明确写着"这个 app 在 Gecko/WebKit 上仍未实测"。

⚠️ **这是动生产安全策略，所以我自己独立量了一遍，没有只看 agent 的表。**
自己写的探针（监听器走 `addInitScript` 隔离世界）打真实线上四条路由：
**app 违规 0**，同时**故意触发的那次被抓到了、注入的颜色没生效**
（停在白色，不是 rebeccapurple）——说明策略确实在拦，不是探针瞎了。
另外截图确认 3D hero、渐变、玻璃面板全部正常，440/1280 横向溢出 0。

⚠️ **顺带证实了 report-uri 这张网是通的**：我那次自检违规
在服务端日志里如实出现了 `[CSP] style-src-attr <- inline (1 report(s))`。
ADR 第 7 节说"改完之后，没被浏览器走到的代码路径就只剩这张网兜着"——
现在知道这张网确实会响。

⚠️ **轨 B 的立意来自事故本身：那次为什么没人发现。**
应用没报错、日志干净、**`/api/health` 全程 200**——因为部署脚本是
**从 VPS 内部走明文 HTTP 打 localhost**，根本不经过 TLS。
所以这个脚本刻意做成"当时会响的那个检查"：从外部走真实 HTTPS、
**证书校验绝不关闭**（脚本还会检测 `NODE_TLS_REJECT_UNAUTHORIZED=0`
并直接判失败——"没验过证书"不能和"证书没问题"印成同一个颜色）。
18 项检查，线上实测全过，证书还剩 88 天。

⚠️ **但它现在还没有人定时跑**——见「待你决策」新增的那条。
一个没人跑的检查器，和没有检查器是一样的。

**2026-09-06 收工：**

- 提交：`bc7b2bc`（轨 A）、`bb19828`（轨 B）+ 本次文档提交，**全部已 push**
- 线上是 `bb19828`（05:43 UTC 部署），本地与线上同码
- 验证：lint / **243 unit**（+45）/ build / 60 api / 93 api-db / 26 e2e（`--workers=2`）
  / 线上 18 项健康检查全过 / 线上真实浏览器 CSP 违规 0 / 日志 0 error
- 本机无服务在跑，两个 worktree 与分支已清理
- 未完项仍是 7 条；**「待你决策」3 → 4 条**（新增：健康检查要不要定时跑、往哪报）

---

### 2026-09-04（第三十三轮，上一轮）：三个窗口并行
...
| | 第三十轮 |
|---|---|
| 做的事 | 6b 带变量的诊断文案可翻译；6c 3D 星图可用键盘到达 |
| 未完项 | **6b、6c 关闭**；新开第 13 条（一个死掉的样式 token） |
| 前端 | `AdminGalaxy.jsx`、`AdminContentHealth.jsx`、`i18nAdmin.js`、`index.css` |
| 服务端 | `contentHealth.js`（每条带变量的 finding 多发一个 `values`） |
| 数据库 | 无变更，本轮没有跑过任何数据库写语句 |
| 部署 | ✅ `e4a19c7`，2026-09-02 15:32 UTC |
| 回滚点 | `/opt/mrright-portfolio.backup-20260902-153200` |

⚠️ **6b 的关键不是那 14 条翻译，是那条防漂移的测试。**
`tests/unit/content-health-i18n.spec.js` 把 `server/contentHealth.js` 当**文本**读，
双向检查：服务端能产出的每个 finding 都得有中日词条；
词典里用到的每个占位符，服务端都得真的发那个值。
**「翻译读起来很顺、但给运维渲染出一个 `{url}`」才是真正会上线的那种失败。**
两个方向都做了**变异验证**（删一条词条、用一个不存在的占位符），都会红。

⚠️ **后台 e2e 的 fixture 原来用的是虚构的 finding code**（`no-model` / `model-missing`），
所以那三张三语截图一直在渲染英文回退、**对词典什么都没验证**。已换成真实 code + `values`。
**以后加 fixture 时注意这一点**：编出来的 code 会让测试静默变成空转。

⚠️ **6c 的焦点层必须是「移出屏幕」，不能是 `display:none` 或 `visibility:hidden`** ——
后两者会把元素**移出焦点顺序**，而那正是要解决的问题本身。
获得焦点时它会回到画布左上角，这样有视力的键盘用户知道自己在哪。

⚠️ **本轮我犯过一次「把构建失败藏起来」**：`npm run build >/dev/null 2>&1 && ...`
让一次真实的构建失败（`bg-black-100/95` 不是有效工具类）被吞掉，
**后面几次测试跑的是旧的 dist**。是 e2e 找不到新元素才暴露出来的。
**以后 build 不要重定向到 /dev/null**，至少留 `| tail -3`。

**2026-09-02 当天收工（这一天做了两轮：第二十九轮和第三十轮，都已部署）：**

- 提交：`9414905`、`4ff5915`（第二十九轮）、`29afe85`（权限白名单）、
  `e4a19c7`、`c3abe0e`（第三十轮）+ 本次收工提交，
  **全部已 push**，`main` 与 `origin/main` 一致
- 线上跑的是 `e4a19c7`（15:32 UTC 部署），**本地与线上同码**
  （线上 `Admin-6FRUTWJW.js` 与本地 dist 已核对），`/api/health` 200
- CI：两个代码提交 × 两个 job（`Web and API` / `C++ App Skeleton`）**全绿**
- 本机没有任何服务在跑（验证用的 express 4321 用完已停），没有残留的临时脚本
- **数据库**：当天只有第二十九轮那一条 DDL（新增可空列），第三十轮一句写语句都没跑。
  收工前复查：`community_posts` 共 1 行、`image_url` 非空 0 行 —— 既有数据未被触碰
- 只用一次性管理会话读过两次（部署脚本的 admin_summary、线上 content-health），用完即撤销
- 未完项：关掉 6b、6c，新开第 12、13 条 → 9 条（净 −1）
- 「待你决策」仍然是 2 条（17 个翻译 key、`crimson-rune-greatsword` 的中文文案串了）
- 没有半途而废的改动挂在本机

**这一天新增的两条操作纪律**（都写在上面各自的 ⚠️ 里，这里只做索引）：

1. **`npm run build` 不要重定向到 `/dev/null`** —— 第三十轮就是这么把一次真实的构建失败
   藏了起来，后续测试跑的是旧 dist。
2. **fixture 里别编 code** —— 后台 e2e 用假的 finding code 空转了很久都没人发现。

**下次开工**：先读这一节。SEO 与后台三语两条线都收口了，
下一轮换方向：路线图第 7 条 C++ SDK，或未完项第 12、13 条那两件小的。
⚠️ 先定怎么验证 —— 第三十轮那条防漂移测试是**做了变异验证才敢信**的。

---

### 2026-09-02（第二十九轮，当天第一轮）：帖子有了自己的配图

**做的是第二十三轮 SEO 衍生清单里最后一条**。项目在第二十四轮有了自己的图，
帖子一直没有 —— 所以每条帖子分享到 Twitter / Discord / Slack / 微信，
配图永远是那张灭火器，无论帖子在讲什么。

| | 第二十九轮 |
|---|---|
| 做的事 | 帖子配图：上传、校验、展示、进 `og:image` 与 JSON-LD |
| 路线图 | 第 6 条衍生的第 3 条 ✅（只剩 `<noscript>` 那条，那条要 SSR） |
| 前端 | `CommunityPage.jsx`、`api.js`、`i18n.js`（三语）、`index.css` |
| 服务端 | 新端点 + 写入校验 + SEO 接线 + schema |
| **数据库** | **有变更**（见下），随服务启动执行 |
| 部署 | ✅ `9414905`，2026-09-02 14:15 UTC |
| 回滚点 | `/opt/mrright-portfolio.backup-20260902-141551` |
...
**纯新增可空列，无回填、无 UPDATE / DELETE / DROP。** 旧代码忽略该列即可运行，
回滚只需退回上一个部署目录。上线后已实测确认：列存在且可空，
`community_posts` 共 1 行、`image_url` 非空的 0 行 —— **没有动过任何既有数据。**

⚠️ **配图上传是独立端点 `POST /api/community/post-images`，不是复用 `/community/uploads`。**
后者发布的是**资源**：会建行、进审核、进资源库列表。配图是页面元素。
复用就会把每张配图都塞进资源库。**以后想「顺手复用上传接口」时先想起这条。**

⚠️ **`imageUrl` 是写入时校验的，不是信任的。** 它必须是本服务存图的
`/uploads/images/<文件名>` 路径，且文件确实存在。
理由：这个值会进 `og:image`，**全世界的链接预览抓取器都会去取它并缓存** ——
那不是一个可以让调用方随便指向任意 URL 的字段，而一个悬空的 og:image
会在每个见过它的客户端里变成永久的破图。
测试里钉了五种拒绝：外部 URL、协议相对 URL、模型路径、路径穿越、不存在的文件。

⚠️ **前端是两步且有顺序**：先传图拿到 URL，再带着 URL 发帖。
反过来（先发帖再补图）会产出「帖子发出去了但图悄悄没了」。

⚠️ **线上还没有任何一条带配图的帖子**（当前只有 1 条帖子，无图）。
端到端已经在 db 契约测试里跑通（传图 → 发帖 → 断言服务端 HTML 里是这张图、
不是灭火器），但**真人发第一条带图帖子时，值得去抓一次那条 URL 的 HTML 确认**。
——和未完项第 6 条（第一个设 handle 的用户）是同一类待观察项。

**第二十九轮收工时**：代码 `9414905` 14:15 UTC 部署，
未完项 8 → 9 条（新开第 12 条：没人认领的配图文件）。
当天随后又做了第三十轮，见上面那一节。

---

### 2026-08-29（第二十八轮，上一轮）：服务端在第一份响应里就说清楚要什么
...
**第二十八轮收工时**：代码 `2512a57` 15:51 UTC 部署，CI 全绿，
未完项 9 → 8 条（第 11 条关闭）。分享链接那条线
（体积 → 预加载 → 请求链深度）三轮做完告一段落。
...
| | 第二十七轮 |
|---|---|
| 做的事 | 冷启动进项目页时，Hero 等面板渲染完再挂载；并把度量固化成脚本 |
| 未完项 | 第 9 条**关闭**（体积那一半已解决，延迟那一半改写成新的第 10 条） |
| 前端 | `src/App.jsx`、`src/sections/Projects.jsx`、`src/components/ProjectDetail.jsx` |
| 服务端 | 一行没动 |
| 数据库 | 无变更，本轮没有跑过任何数据库写语句 |
| 部署 | ✅ `4f1af12`，2026-08-28 15:46 UTC（当天第二次部署，第一次是 `c0c4d6b` 15:16） |
| 回滚点 | `/opt/mrright-portfolio.backup-20260828-154600` |

⚠️ **唯一可信的指标是「面板出现前下载了多少 JS」，不是耗时。**
`1488 KB → 467 KB`。耗时在我这条链路上根本没有分辨率 ——
同一个 217 KB 的入口 bundle，上午 1.8 秒、下午 9.3 秒。
`scripts/measure-project-link.mjs` 的输出里现在写着这句话。

⚠️ **触发时机试了三次，前两次都是「看起来生效、其实没生效」。**
1. `requestIdleCallback`（第二十六轮）—— 网络等待期间浏览器就是空闲的，提前触发。
2. `document.readyState` / window `load` 事件（本轮第一版）——
   **React 的首次渲染是被调度的**，HomePage 挂载时 `load` 往往已经触发过了，
   等于没延迟。本机 trace 里 Hero 和 three.js 在 9.6s 与其他 chunk 一起被请求，
   就是这么露馅的。
3. **面板自己发信号**（`ProjectDetail` 挂载时调 `onReady`）—— 这个才是真的。

⚠️ **兜底定时器一开始写了 6 秒，线上 waterfall 抓到它在帮倒忙**：
慢链路上它 11.3s 就放行了 Hero，而面板 12.9s 才出现 ——
正好把 three.js 放回它该躲开的那段抢占里。现在是 15 秒。
**链路越慢，Hero 越该多等**，这个定时器只是为了让延迟不会变成永久。

⚠️ **别把部署输出接进 `head`。** 本轮 `npm run deploy:vps | tee … | grep … | head -5`
让部署脚本吃了 EPIPE **中途挂掉**（`exit=35`）。
好在服务当时仍在跑上一版、`/api/health` 在 VPS 本地 200，没有造成损害。
**要看部署输出就重定向到文件，事后再 grep。**
（上一轮的教训是「别为了看一行重跑部署」，这一轮是它的近亲。）

**2026-08-28 当天收工（这一天做了两轮：第二十六轮和第二十七轮，都已部署）：**

- 提交：`cc52af3`、`4ed356a`、`6523db9`、`51d8c90`（第二十六轮）、
  `c0c4d6b`、`4f1af12`、`8ea5101`（第二十七轮）+ 本次收工提交，
  **全部已 push**，`main` 与 `origin/main` 一致
- 线上跑的是 `4f1af12`（15:46 UTC 部署），**本地与线上同码** ——
  线上和本机 `dist` 都是 `index-8DOkuOUg.js`，已逐字核对；`/api/health` 200
- CI：两轮的代码提交（`cc52af3` / `c0c4d6b` / `4f1af12`）两个 job 全绿
- 本机没有任何服务在跑，没有残留的临时脚本
  （反事实实验、waterfall 追踪那些 `.tmp-*.mjs` 都是用完即删；
  **唯一留下来的是 `scripts/measure-project-link.mjs`，那是有意提交的**）
- **两轮都没有跑过任何数据库写语句**
- 未完项**仍然是 9 条**：第 9 条关闭，同时开了第 11 条（请求链深度）
- 「待你决策」仍然是 2 条（17 个翻译 key、`crimson-rune-greatsword` 的中文文案串了）
- 没有半途而废的改动挂在本机

**下次开工**：先读这一节，再看下面「下一轮我建议先做的」。
**最具体的是新开的未完项第 11 条**（给项目路由的 HTML 补 `modulepreload`，
砍掉两层串行的依赖发现）——
⚠️ **动手前先定好怎么验证：量请求轮数，别量墙钟时间。**
这两天连着栽在这件事上两次（第二十六轮量错环境、第二十七轮量不出效应）。

---

### 2026-08-28（第二十六轮，当天第一轮）：全站不再预加载 971 KB 的 three.js
...
**第二十六轮收工时**：代码 `cc52af3` 05:11 UTC 部署，CI 两个 job 全绿，
未完项 10 → 9 条（第 10 条关闭，第 9 条只做掉一半）。
当天随后又做了第二十七轮，把第 9 条剩下的一半做完了，见上面那一节。
⚠️ 那一轮被否掉的 `useDeferredHero` 是**量完删掉的**，
而第二十七轮换了触发时机、换了度量环境之后**它是对的** —— 见上面。
...
**线上运行 `b8212af`（2026-09-06 06:33 UTC 部署，已逐项验证）。**
第三十六轮上线的只有 i18n 词典删除（51 条未渲染的字符串）；
同轮还改了一行生产数据（项目中文文案）和删了 232MB 孤儿上传文件，那两项不需要部署。
回滚到第三十六轮之前：`/opt/mrright-portfolio.backup-20260906-063349`。

（同日第三十五轮：`e6b304e`，06:01 UTC。）

**第三十五轮线上是 `e6b304e`（已逐项验证）。**
第三十五轮加的是失败告警脚本（不影响运行时）+ VPS 上的 cron。
回滚到第三十五轮之前：`/opt/mrright-portfolio.backup-20260906-060144`。

（同日第三十四轮：`bb19828`，05:43 UTC 部署，收紧了 CSP。）

**第三十四轮线上是 `bb19828`（已逐项验证）。**
第三十四轮上线的是收紧后的 CSP（`style-src` 去掉 `'unsafe-inline'`）
以及生产健康检查脚本（脚本不影响运行时）。
回滚到第三十四轮之前：`/opt/mrright-portfolio.backup-20260906-054258`。

（上一轮：第三十三轮 `1b599ee`，2026-09-04 15:32 UTC 部署。）

**第三十三轮线上是 `1b599ee`（已逐项验证）。**
...
（上一轮：第二十九轮 `9414905`，同日 14:15 UTC 部署，含那条新增列的 DDL。）

**第二十九轮线上是 `9414905`（已逐项验证）。**
第二十九轮**前端、服务端、数据库都有改动**（自第十二轮以来第一次动 schema，
只有一条新增可空列的 DDL）。
回滚到第二十九轮之前：`/opt/mrright-portfolio.backup-20260902-141551`。

（上一轮：第二十八轮 `2512a57`，2026-08-29 15:51 UTC 部署。）

**第二十八轮线上是 `2512a57`（已逐项验证）。**
...
所以 `...-151631` 那份装的是第二十七轮的第一版，不是干净的回滚点。

（上一轮：第二十六轮 `cc52af3`，05:11 UTC 部署，只改了构建。）
...
```text
/opt/mrright-portfolio.backup-20260906-063349   第三十六轮之前 ← 要回滚就用这个
/opt/mrright-portfolio.backup-20260906-060144   第三十五轮之前
/opt/mrright-portfolio.backup-20260906-054258   第三十四轮之前（回到带 unsafe-inline 的 CSP）
```

⚠️ **回滚应用不会把删掉的孤儿文件找回来**：那些文件在 `public/uploads/`，
是持久目录、不随部署替换。要找回只能从
`/root/orphaned-uploads-20260906.tar.gz` 解。

⚠️ **`/usr/local/bin/mrright-health-check.sh` 和 crontab 不在这些备份里**——
它们在 `/opt` 之外，回滚应用不会动到它们（这正是把 wrapper 放在外面的原因）。
原 crontab 备份在 `/root/crontab.backup-*`。

⚠️ **回滚过第二十九轮的话，`community_posts.image_url` 那一列会留在库里。**
那没有关系：旧代码不读它。**不要为了回滚去 DROP COLUMN。**

⚠️ **最新那份不是回滚点**：当天部署了两次，第二次备份的是第一次的产物。
要回滚先 `ls -dt` 并核对目录里的代码，别照名字猜。
...
9. ~~**分享一条项目链接要和 971 KB 的 three.js 抢带宽**~~ ——
   **2026-08-28 第二十六 + 二十七轮做完了。**
   第二十六轮拿掉了全站的 `modulepreload`（根因是 vite 的 preload-helper 被分进了
   three chunk）；第二十七轮让冷启动进项目页时 Hero 等面板渲染完再挂载。
   **面板出现前下载的 JS：1488 KB → 467 KB**，关键路径上的 three.js 归零。
   ⚠️ **耗时没有跟着一起变好，这一点要说清楚**：线上中位数仍在 8～10 秒，
   但那已经不是体积决定的了（见新的第 10 条）。而且**我这条链路量不出 3 秒的差别** ——
...
6b. ~~**后台三语里有两处仍然是英文**~~ —— **2026-09-02 第三十轮做掉了一处半。**
   带变量的 content-health hint 已经可翻译：服务端多发一个 `values`，
   前端用占位符拼回句子，中日各补 14 条词条，
   并且有 `tests/unit/content-health-i18n.spec.js` 双向防漂移。
   ⚠️ **审计日志里的 action 名（`visitor.profile_disabled`）仍然是原样输出的**，
   那是一个稳定的机器标识符，直接翻译会让它和数据库里的值对不上；
   要做就是给它一张显示名映射表，而不是改服务端发的值。
   （量很小，且运维看得懂，所以没有归到未完项里。）

6c. ~~**3D 运营星图不可 Tab 到**~~ —— **2026-09-02 第三十轮做掉了。**
   画布上叠了一层真正的按钮，平时移出屏幕、获得焦点时回到画布左上角。
   ⚠️ **不能改成 `display:none` 或 `visibility:hidden`** —— 那会把它移出焦点顺序，
   正是原来的毛病。回归测试在 `tests/e2e/admin-console.spec.js`，
   断言的是「能被 focus 且能用 Enter 触发」，不是「能被点击」。
...
**当前开着的只有 1 条**（其余都已结清，划掉的保留作记录）。

- **外部 uptime 服务还没加** —— 这是第三十五轮那条的后半截，单独拎出来免得被埋掉。
  cron 已经在跑了（每天 12:00 UTC，失败发信），但**它跑在被监控的那台机器上**：
  整机挂了、网络断了、systemd 起不来，它都不会响，而那时网站对访客就是完全打不开。
  ⚠️ **这个洞 cron 补不上，只能由机器外面的东西补。**
  UptimeRobot 之类免费档就够，盯 `https://mrright.blog/api/health`，五分钟能配好。
  **要你的账号和收件箱，所以得你来。**

- ~~**健康检查脚本没有任何东西定时跑它**~~ —— **2026-09-06 第三十五轮做掉了**
  （你回「由你决定」，我选了 VPS cron + 失败发信，理由写在上面那一节）。
  现在每天 12:00 UTC 跑一次，只在失败时发信到你的邮箱。
  ⚠️ **剩下的那半仍然建议你自己加：外部 uptime 服务。**
  cron 跑在被监控的那台机器上，**整机挂了它不会响**，
  而这恰恰是 cron 补不上的那个洞。UptimeRobot 之类免费档就够，
  盯 `https://mrright.blog/api/health` 即可，五分钟能配好。

- ~~**机场那套域名有两张 TLS 证书已过期两个多月**~~ ——
  **2026-09-06 你的决定：那是废弃证书，不用管。** 记录在此备查：
  `hy.mrright.blog`（6 月 29 日过期）、`suijp.mrright.blog`（7 月 28 日过期）
  由 acme.sh 管理，与网站主域的 certbot 分工不冲突（nginx 网站 vhost 用的是 certbot 那份）。
  **未做任何改动**，acme.sh 的 crontab 条目也原样保留。
  ⚠️ 若哪天想彻底清掉每天 10:55 的无效续期尝试，`acme.sh --remove` 即可，
  但那会动到机场那套东西，仍属你的决定。

- ~~**线上 `public/uploads/` 里有 231.8 MB（24 个文件）没有任何数据库行引用**~~ ——
  **2026-09-06 第三十六轮按你的指示删除了**，`uploads` 从 252MB 降到 20MB。
  ⚠️ **删之前先归档了**：`/root/orphaned-uploads-20260906.tar.gz`（223M，在 `/opt` 之外）。
  归档校验过才动手（解开那个源文件比对 sha256 一致）。
  **那个被 `server/content.js` 注释点名的灭火器模型源文件
  `1781017698552-tl-miehuoqi.glb` 就在归档里**，要取回：
  `tar -xzf /root/orphaned-uploads-20260906.tar.gz -C /opt/mrright-portfolio/public ./uploads/models/1781017698552-tl-miehuoqi.glb`
  ⚠️ 归档没有异地副本（沿用你 2026-08-12「不需要异地备份」的决定）。
  删后复验：孤儿归零，剩余 7 个文件正好是四个项目在用的资源，线上 URL 全部 200。

- ~~**`crimson-rune-greatsword` 的中文文案是另一个项目的**~~ ——
  **2026-09-06 第三十六轮按你的指示改成与英文对应了。**
  实际串了**三栏**（比最初报告的多一栏）：`titleZh`、`summaryZh`、
  **`workflowZh`**，都改成了与 `title`/`summary`/`workflow` 一致的长剑文案。
  线上复核：四个项目中文标题不再重复，祭坛那个项目未被波及。
  ⚠️ **`formatZh` 仍是英文 `"GLB model"`**（`format` 基础栏是 `"Hand-painted scene"`）——
  那属于"没翻译"不是"串项目"，不在这次指示范围内，留着没动。

- ~~**17 个「写了三份翻译、谁也不渲染」的 i18n key，要不要删**~~ ——
  **2026-09-06 第三十六轮按你的指示删除了**（`b8212af`），三份词典共 51 条字符串。
  ⚠️ **那道闸没有跟着消失，反而更严了**：`tests/unit/i18n-usage.spec.js`
  的钉住清单现在是空数组，断言从"没有新增的未渲染 key"变成
  "字典里一个未渲染的 key 都不该有"。做过变异验证（加回一个立刻变红）。
...
6. ~~SSR / 预渲染 SEO~~ —— **2026-08-22 第二十三轮做完并上线了**，
   ...
   - ~~**分享卡片的图还是全站一张灭火器**~~ —— **2026-09-02 第二十九轮做完了。**
     帖子现在可以带配图，进 `og:image`、进 JSON-LD、也进 `<noscript>`；
...
9. ~~CSP 还可以再紧一格：`style-src` 现在带 `'unsafe-inline'`~~ ——
   **2026-09-06 第三十四轮已落地上线**（`bc7b2bc`）。线上现在是
   `style-src 'self' https://fonts.googleapis.com`。
   门 1（登录态后台，含 3D galaxy）和门 2（有内容的社区列表）都用现成的 fixture
   harness 关掉了，各自违规为 0；门 3（非 Chromium 引擎）**没测也没装浏览器**，
   ADR 里留着。落地后我自己独立复量过线上四条路由，并确认 report-uri 这张网会响。
   ⚠️ **`tests/api/contract.spec.js` 里现在钉着 `style-src` 不得含 `'unsafe-inline'`、
   也不得出现 `'unsafe-hashes'`，且必须保留 fonts.googleapis.com**
   （字体那半是承重的：去掉它 `src/index.css:1` 的 `@import` 会在每条路由上违规）。
   以下为当初调研的结论，保留作为记录：
   **原假设是错的**——"要去掉得先上 nonce 或 hash"，
...
- ⚠️ **这台机器跑不动默认并发的 e2e**（2026-09-04 第三十三轮量到，当时口头说了没记）：
  `admin-console` + `site-routing` 两套一起跑（26 条，8 workers 默认并发）
  会有 4 条超时——每条都要起一个跑 WebGL 的无头 Chromium。
  **`--workers=2` 时 26/26 全过**，单独跑任一套也全过。
  **这不是回归，是机器容量**；CI 根本不跑这两套（它们默认指向生产 URL），
  所以只影响本机。以后看到这几条红，先降并发再怀疑代码。
...
未完项 6b 和 6c，都是第二十轮把后台做成三语时留下的。**公开站点一行没动。**

### 6b：带变量的诊断文案

固定文案早就有 i18n 机制（`finding.<code>.<field>`），**卡住的只是句子里嵌了值的那些**：

```text
Nothing resolves at /models/x.glb. Re-upload the model or fix the path.
The model is 42.4 MB, which is heavy for a phone.
The row says 1024 bytes; the file is 2048.
```

这种句子查不了词典 —— 值是拼死在里面的。

做法：服务端**继续发那句英文**（CLI 校验器打的就是它，也是没有词条时的回退），
同时把值单独发一份：
...
前端 `findingText` 把 `values` 交给翻译器，词条里用 `{url}` 占位。
中日各补 14 条。**英文不需要词条**：查不到就回退到服务端那句，正是它。

### 这一轮真正的产出是那条防漂移测试

`tests/unit/content-health-i18n.spec.js`，把 `server/contentHealth.js` **当文本读**
（import 只会拿到某个 fixture 恰好触发的那些 finding，
而漏翻译的恰恰是没人 fixture 得到的罕见项），双向检查：

1. 服务端能产出的每个 finding，中日都得有词条；
2. **词典里用到的每个占位符，服务端都得真的发那个值。**

第 2 条防的是更难看的那种失败：**翻译读起来很顺，但给运维渲染出一个 `{url}`。**

⚠️ **做了变异验证才敢信它**（第二十五轮那个假探针的教训）：
删掉一条中文词条 → 红；把词条里的占位符改成服务端不发的名字 → 红，
并且报出 `zh: finding.content-missing.hint -> {nosuchvalue}`。

### 顺带发现：后台 e2e 的三语截图一直在空转

fixture 里的 finding code 是编的（`no-model` / `model-missing` / `missing` / `draco`），
真实服务端根本不产出这些 code。于是那三张三语截图**一直渲染的是英文回退**，
对词典什么都没验证。已换成真实 code + `values`，并加了一条断言：
中文控制台里那条 finding 必须是中文、**且路径还在句子里**、
既没有残留的 `{url}`，也没有英文原句。

### 6c：星图的键盘可达性

canvas 里的对象不在焦点顺序里，所以 3D 模式下整张运营图**键盘完全够不着**。
现在画布上叠了一层真正的 `<button>`（同样的标签、同样的跳转），
平时移到屏幕外，**任一个拿到焦点时整层回到画布左上角**。

⚠️ **必须是「移出屏幕」，不能是 `display:none` / `visibility:hidden`** ——
后两者会把元素移出焦点顺序，那就是原来的毛病本身。

回归测试断言的是「**能被 focus、且 Enter 能触发跳转**」，而不是「能被点击」——
点击对一个不在焦点顺序里的元素同样成立，那样的测试等于没写。

### 我在本轮犯的一个错

`npm run build >/dev/null 2>&1 && ...` —— 一次**真实的构建失败被吞掉了**
（`bg-black-100/95` 不是有效工具类），
而后面几次测试**跑的是旧的 dist**，直到 e2e 找不到新元素才暴露。
**以后 build 不要重定向到 /dev/null**，至少留 `| tail -3`。

由此还带出未完项第 13 条：`bg-black-100` 这个 token 根本不存在，
所以 `src/main.jsx` 里那张错误卡片的背景**从来没生效过**。
...
|---|---|
| `npm run lint` | 通过 |
| `npm run test:unit` | **177 通过**（新增 4 条，新文件 `content-health-i18n.spec.js`） |
| 变异验证 | 两个方向都能让新测试变红，改回即恢复 |
| `npm run build` | 通过（修掉了本轮自己弄坏的那次） |
| `npm run test:api` / `test:api:db` | 60 / 92 通过 |
| `npm run test:openapi` 等全部 verifier | 通过 |
| 本机 `admin-console.spec.js` | **8 通过**（新增 2 条：中文 finding、键盘可达） |
| 本机 `site-routing.spec.js` | 15 通过 |
| `git diff --check` | 通过 |
| 部署前备份 | `/opt/mrright-portfolio.backup-20260902-153200` |
| 线上 | 七条路由 200；`Admin-6FRUTWJW.js` 与本地一致；日志 0 error |
| 线上 content-health | 一次性会话读取：**0 条 finding**（内容本身是健康的），会话已撤销 |

⚠️ **线上目前一条 finding 都没有**，所以 6b 在生产上没有可渲染的样本 ——
证明落在 fixture 驱动的三语 e2e 上。这不是坏消息：0 条 finding 说明
第二十九轮加的配图列没有把任何资源检查弄坏。

### 改动的文件

- `server/contentHealth.js`（每条带变量的 finding 多发 `values`）
- `src/components/admin/AdminContentHealth.jsx`（把 values 交给翻译器）
- `src/components/admin/AdminGalaxy.jsx`（焦点层）
- `src/lib/admin/i18nAdmin.js`（中日各 14 条）
- `src/index.css`（焦点层样式）
- `tests/unit/content-health-i18n.spec.js`（新）
- `tests/e2e/admin-console.spec.js`（真实 code 的 fixture + 2 条新用例）

commit：`e4a19c7`（代码）+ 本次文档提交
回滚点：`/opt/mrright-portfolio.backup-20260902-153200`

## 2026-09-02（第二十九轮）：帖子配图

第二十三轮 SEO 衍生清单的最后一条（除了要 SSR 的那条）。
**第十二轮以来第一次动数据库 schema。**
...
随服务启动执行，和第九、十一轮的做法一致。
**纯新增可空列：无回填、无 UPDATE、无 DELETE、无 DROP。**
上线后实测：列存在且可空，`community_posts` 共 1 行、`image_url` 非空 0 行 ——
**既有数据一个字节都没动。**

⚠️ **回滚不需要 DROP COLUMN**，旧代码不读这一列。

### 为什么另开一个上传端点

`POST /api/community/post-images`，而不是复用 `/api/community/uploads`。

因为那两件事根本不是一回事：

| | `/community/uploads` | `/community/post-images` |
|---|---|---|
| 语义 | 发布一个**资源**给别人下载 | 一张**页面配图** |
| 产物 | 一行记录 + 审核状态 + 进资源库列表 | 一个文件 + 一个 URL |
| 复用的代价 | —— | 每张配图都会进资源库 |

端点只负责存文件、返回 URL；**帖子创建时才算认领**。

### `imageUrl` 是校验的，不是信任的

写入时必须满足：是本服务存图的 `/uploads/images/<文件名>` 路径、
文件名不含路径分隔符、**且文件确实存在**。

理由写在代码注释里，这里再说一遍：**这个值会进 `og:image`**，
全世界的链接预览抓取器都会去取它并缓存。
那不是一个可以让调用方指向任意 URL 的字段；
而一个悬空的 og:image，会在每个见过它的客户端里变成永久破图。

测试钉了五种拒绝：外部 URL、协议相对 URL（`//evil.example/...`）、
模型路径、路径穿越、不存在的文件。

### 前端

- 发帖表单加「配图（可选）」，控件**复用资源上传那个 `asset-upload-control`** ——
  原生 file input 会渲染自己的英文标签，那会是整页唯一一处没翻译的字。
- 选中后本地预览（`URL.createObjectURL`，替换和清空时都 revoke）。
- 提交是**两步且有顺序**：先传图拿 URL，再带 URL 发帖。
  反过来会产出「帖子发出去了、图悄悄没了」。
- 列表卡片和详情页都渲染配图；没有配图的帖子排版和以前一致。
- 三语 key 各 4 个（`communityChooseImage` / `communityPostImage` /
  `communityPostImageHint` / `communityPostImageRemove`）。

### SEO

配图进三个地方：`og:image`、`DiscussionForumPosting` 的 `image`、
以及 `<noscript>` 里的 `<img>`。没有配图的帖子仍然回落到站点图。
...
|---|---|
| `npm run lint` | 通过 |
| `npm run test:unit` | 173 通过（新增 4 条：head 用自己的图 / 回落 / 图谱 / noscript） |
| `npm run build` | 通过 |
| `npm run test:api` | 60 通过 / 13 skipped |
| `npm run test:api:db` | **92 通过**（新增 6 条） |
| `npm run test:openapi` | 通过（新端点 + `imageUrl` 都进了契约） |
| 其余 verifier | 全部通过 |
| `git diff --check` | 通过 |
| 本机 express + `site-routing.spec.js` | 15 通过 |
| 440px / 1280px 截图 | 表单与卡片密度正常，横向溢出 0 |
| 部署前备份 | `/opt/mrright-portfolio.backup-20260902-141551` |
| 线上 | 七条路由 200；`image_url` 列已生效；旧帖 `imageUrl: ""` 且 og:image 回落到站点图；新端点无凭证返回 401；日志 0 error |

**端到端那条在 db 契约测试里**：传图 → 用返回的 URL 发帖 →
断言服务端 HTML 的 `og:image` 是这张图、**且不含 `fire-extinguisher.png`**，
再断言 JSON-LD 里 `DiscussionForumPosting.image` 也是它。

⚠️ **线上还没有真人发过带配图的帖子**（当前只有 1 条帖子，无图）。
第一条出现时，值得抓一次那条 URL 的 HTML 确认 ——
和未完项第 6 条（第一个设 handle 的用户）是同一类待观察项。

### 顺手记下的一条新未完项

配图传了但没发帖（填一半关掉页面），文件会留在 `public/uploads/images/` 无人认领。
详见未完项第 12 条，**里面写了清扫任务必须同时查哪几张表**，少查一张就会删掉在用的图。

### 改动的文件

- `server/postgres/schema.js`、`mappers.js`、`communityStore.js`、`adminStore.js`
- `server/index.js`（新端点 + `normalizePostImageUrl`）
- `server/seo.js`（og:image / JSON-LD / noscript）
- `src/pages/CommunityPage.jsx`、`src/lib/api.js`、`src/lib/i18n.js`、`src/index.css`
- `docs/openapi/api-v1.yaml`
- `tests/unit/seo.spec.js`、`tests/api/contract.db.spec.js`

commit：`9414905`（代码）+ 本次文档提交
回滚点：`/opt/mrright-portfolio.backup-20260902-141551`
...
未完项第 9 条剩下的那一半。第二十六轮定的规矩是「**先解决怎么度量再动手**」，
这一轮就是照着做的 —— 结果是：**度量方法本身才是这一轮真正的产出。**

### 一、先把度量做对

第二十六轮凭直觉写了「延迟挂载 Hero」，在本机节流下量不出增益，于是删了。
本轮第一件事是问：**那个度量为什么会得出那个结论？**

答案是**本机 express 是 HTTP/1.1**。Chrome 在 HTTP/1.1 下对同一主机最多六条连接、
按发现顺序排队，面板的分块本来就排在前面，three.js 抢不走什么。
**线上 nginx 是 HTTP/2，所有请求多路复用共享带宽** —— 争抢只存在于真实那一侧。

所以度量必须打真实那一侧。做法是**反事实对照**，不需要部署：

- A 组：正常冷启动
- B 组：一模一样，只是把 `three-*.js` 的响应**推迟 20 秒**
  （= 「如果 3D 引擎不参与竞争」的上界）
- 交替跑，让链路漂移平摊到两组

⚠️ **第一版是把请求 abort 掉的，那组数据全是废的**：
动态 import 失败 → `src/main.jsx` 的 chunk 错误处理**把页面刷新了**，
量到的是一个刷新循环（>60s，而且字节数反而更多）。改成延迟才对。

部署前的线上结果（5 对）：

```text
正常          中位 10.1s   (7.4-17.7s)
three.js 延后  中位  7.2s   (6.7-8.0s)
```

**值约 3 秒，而且把 10 秒的方差压成 1.3 秒。** 于是决定做。

这套东西固化成了 `scripts/measure-project-link.mjs`，以后可以直接复跑。

### 二、触发时机试了三次

| | 触发条件 | 结果 |
|---|---|---|
| 第二十六轮 | `requestIdleCallback` | ❌ 网络等待期间浏览器就是空闲的，提前触发 |
| 本轮第一版 | `document.readyState` / window `load` | ❌ **React 首次渲染是被调度的**，HomePage 挂载时 `load` 往往已经过去了 |
| 本轮最终 | **面板挂载时自己发信号** | ✅ |

第二种是被本机 trace 抓出来的：Hero 和 three.js 在 9.6s 和其它分块**一起**被请求，
说明 `heroReady` 首帧就是 true。

最终实现：`ProjectDetail` 挂载时调 `onReady`，
经 `Projects` 传回 `App` 的 `useDeferredHero`。三个文件各加一个 prop。
**只对冷启动生效** —— 从网格点进来时 Hero 早就挂着了，
卸载它就等于拆掉第二十四轮特意保住的 3D 场景。

### 三、兜底定时器一开始是帮倒忙的

第一版写了 6 秒。**线上 waterfall 抓到它在慢链路上 11.3s 就放行了 Hero，
而面板 12.9s 才出现** —— 正好把 three.js 塞回它该躲开的那段争抢里，
而且是在最需要它躲开的连接上。改成 15 秒：
**链路越慢，Hero 越该多等**，这个定时器只是为了让延迟不会变成永久。

### 四、结果，以及一个必须说清楚的「没有变好」

| | 部署前 | 部署后 |
|---|---|---|
| **面板出现前下载的 JS** | **1488 KB** | **467 KB** |
| 关键路径上的 three.js | 971 KB | **0** |
| 线上中位耗时 | 10.1s | 8.4～9.9s |

**体积那一半彻底解决了；耗时几乎没动。**
不是改动没生效（字节数是确定性的、和链路无关的证据），
而是**耗时已经不由体积决定了**。

⚠️ **而且我这条链路量不出 3 秒的差别。** 同一个 217 KB 的入口 bundle，
当天上午 1.8 秒、下午 9.3 秒（25～40 KB/s）。
部署后跑同一个脚本，两组都是 467 KB，中位数分别是 8.4s 和 11.8s ——
**噪声比效应大**。所以脚本的输出里现在写了一句：**看 KB 那一列。**

那 8～10 秒现在花在哪，线上 waterfall 说得很清楚：

```text
0.01s  请求 HTML
0.42s  HTML 到手
0.50s  请求入口 bundle + react-vendor
4.99s  入口 bundle 下完        ← 这里之前什么都没发生
5.27s  才开始请求 Projects 等分块
5.78s  才开始请求 ProjectDetail 分块
12.9s  面板出现
```

**四层串行的依赖发现**，每一层都要等上一层解析完才知道下一步要什么。
这就是新的未完项第 11 条，下一轮的杠杆在那里。

### 五、部署时的一个操作失误

`npm run deploy:vps | tee … | grep … | head -5` —— **`head` 提前关掉管道，
部署脚本吃了 EPIPE 中途挂掉**（`exit=35`）。
当时服务仍在跑上一版、VPS 本地 `/api/health` 200，**没有造成损害**，
确认之后重新干净地部署了一次（所以当天有两次部署）。
**看部署输出就重定向到文件、事后再 grep。**
（上一轮的教训是「别为了看一行重跑部署」，这条是它的近亲。）
...
| `npm run build` | 通过 |
| `npm run test:api` | 58 通过 / 13 skipped |
| `npm run test:api:db` | 86 通过 / 0 失败 |
| `npm run test:openapi` / `deploy-backup` / `deploy-script` / `admin-totp` / `content-health` / `verify:visitor-studio` | 全部通过 |
| `git diff --check` | 通过 |
| 本机 express + `site-routing.spec.js` | **15 通过**（新增 2 条：断言请求顺序，而不是耗时） |
| 线上 | 七条路由全 200，modulepreload 只剩 `react-vendor`，JSON-LD 图谱仍在，日志 0 error |
| 线上确定性指标 | 面板出现前 **467 KB**（部署前 1488 KB） |

新增的两条 e2e **断言的是顺序不是时间**：
「`three-core` 不得在 `ProjectDetail` 分块之前被请求」以及「Hero 最终仍会出现」。
拿耗时做断言只会造出一个必然会红的测试。

### 改动的文件

- `src/App.jsx`（`useDeferredHero`）
- `src/sections/Projects.jsx`（透传一个 prop）
- `src/components/ProjectDetail.jsx`（挂载时 `onReady`）
- `scripts/measure-project-link.mjs`（新）
- `tests/e2e/site-routing.spec.js`

commit：`c0c4d6b`（实现）、`4f1af12`（兜底定时器修正）+ 本次文档提交
回滚点：`/opt/mrright-portfolio.backup-20260828-154600`
