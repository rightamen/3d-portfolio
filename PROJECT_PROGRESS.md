...
## 下次从这里继续（截至 2026-09-06 第三十四轮收工）

### 2026-09-06（第三十四轮）：CSP 收紧落地 + 给"不会报错的故障"装一个探子
...
**线上运行 `bb19828`（2026-09-06 05:43 UTC 部署，已逐项验证）。**
...
```text
/opt/mrright-portfolio.backup-20260906-054258   第三十四轮之前 ← 要回滚就用这个（回到带 unsafe-inline 的 CSP）
...
- **健康检查脚本做好了，但现在没有任何东西定时跑它**（2026-09-06 第三十四轮）。
  `npm run check:production` 是只读的、18 项、线上实测全过、失败时退出码非 0，
  ——**但一个没人跑的检查器，和没有检查器是一样的。**
  9 月 4 号那次证书过期，缺的从来不是"能不能查出来"，是"有没有人在查"。
  三条路，代价和可靠性依次上升：
  1. **VPS 上加一条 cron**（比如每天一次）。最省事，但**要想清楚失败了报给谁**——
     报到没人看的日志里等于没报。本站 SMTP 走 Resend 且已实测直达收件匣，
     理论上可以让失败时发一封信，但脚本目前不会发信，得再写一点。
  2. **外部 uptime 服务**（UptimeRobot 之类）盯 `/api/health` + 证书。
     好处是**它在机器外面**——VPS 整个挂了它也会叫，cron 不会。
  3. **保持手动**，每次部署后和演练时跑一次。
  ⚠️ 我没有替你选，因为这牵扯到"报警往哪送"，那是你的收件匣。
  ⚠️ 另外注意第 1 条的盲区：cron 跑在被监控的那台机器上，
     机器本身宕了就什么都不会发生。
