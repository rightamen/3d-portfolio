...
## 下次从这里继续（截至 2026-09-06 第三十五轮收工）

### 2026-09-06（第三十五轮）：让检查器真的有人跑（你把选择权交给我，我选了方案 1）
...
**线上运行 `e6b304e`（2026-09-06 06:01 UTC 部署，已逐项验证）。**
...
```text
/opt/mrright-portfolio.backup-20260906-060144   第三十五轮之前 ← 要回滚就用这个
/opt/mrright-portfolio.backup-20260906-054258   第三十四轮之前（回到带 unsafe-inline 的 CSP）
/opt/mrright-portfolio.backup-20260904-153159   第三十三轮之前
```
...
- **机场那套域名有两张 TLS 证书已过期两个多月**（2026-09-06 第三十五轮装 cron 时撞见）。
  `hy.mrright.blog`（6 月 29 日过期）和 `suijp.mrright.blog`（7 月 28 日过期）；
  `jp.mrright.blog` 有效到 10 月 28 日；`mrmyself.life` 找不到证书文件。
  这四个由 **acme.sh**（crontab 里 10:55 那条）管理，
  和网站主域的 certbot 是**分工不是冲突**——nginx 网站 vhost 加载的是 certbot 那份。
  ⚠️ **我没有动它们**：这是机场节点的域名，你 2026-08-11 明确定过不动机场基础设施，
  而续期会触发验证流程、有可能影响正在用节点的人。要不要处理、什么时候处理，是你的决定。
  ⚠️ 如果那两个节点其实早就不用了，那更简单：`acme.sh --remove` 掉即可，
  免得每天 10:55 白跑并留下失败记录。
