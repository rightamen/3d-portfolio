...
- `scripts/validate-openapi.mjs` — OpenAPI contract artifact 自动校验（YAML、`$ref`、strict envelope、error code enum）
...
已接入自动校验（2026-07-05，checklist #9 工程化）：

- 新增 `scripts/validate-openapi.mjs` 与 `npm run test:openapi`，使用项目现有依赖树中的 `js-yaml`，未新增重量级依赖。
- 校验内容：
  - `docs/openapi/api-v1.yaml` 可解析且存在 `openapi`、非空 `paths`、`components.schemas`。
  - `components.schemas.ResponseEnvelope`、`ApiError`、`Pagination` 必须存在。
  - 全部本地 `$ref` 必须解析到本文件中存在的位置。
  - 所有 `application/json` response schema 必须通过 `ResponseEnvelope` 或 `ErrorEnvelope`，避免裸返回业务模型。
  - strict `/api/v1` envelope 顶层固定为 `data`、`pagination`、`error`，且 `additionalProperties: false`。
  - `components.schemas.ApiErrorCode.enum` 与 `server/responses.js` 的 `API_ERROR_CODES` 逐一致，缺失或多余都会失败。
...
| 9 | OpenAPI spec 抽取 + 漂移检测 | ✅ 2026-07-05 | §18；`docs/openapi/api-v1.yaml` 初稿已生成（38 paths/42 ops，错误码枚举与 `API_ERROR_CODES` 逐一致）。`scripts/validate-openapi.mjs` + `npm run test:openapi` 已接入自动校验：YAML、`$ref`、response envelope、strict envelope 顶层键、error code enum |
