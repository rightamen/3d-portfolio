...
11. 下一步：真实 backend implementation / dependency strategy。HTTP backend 必须只消费 `/api/v1/*` strict envelope；需要在 Qt Network 与 libcurl 之间做依赖策略决策；Qt/QML 应在 SDK 边界稳定后接入。
