...
- `cpp-app/` — C++ cross-platform prototype skeleton（CMake + SDK headers + smoke CLI + mock-driven SDK contract tests），已创建；当前已有 opt-in Qt/QML desktop shell 第一批，不含完整 Qt UI、缓存、下载器或打包实现。C++ SDK strict envelope parser boundary 已将 nlohmann/json 设为默认 backend，temporary parser 仍保留为显式 no-dependency fallback。
...
21. Qt/QML desktop shell 第一批。✅ 已新增 opt-in `MRRIGHT_ENABLE_QT_UI=OFF` CMake option 和 `cpp-app/app/ui/qt` shell：`main_qt.cpp` 创建 `QGuiApplication` / `QQmlApplicationEngine`，`Main.qml` 显示 app name、SDK version、`/api/v1` strict mode 和 no-network status，`AppController` 只暴露只读信息。SDK core 继续 Qt-free，Qt/QML 层只在 `app/ui/qt`，本批不登录、不读 token、不访问 API、不调用 admin、不做 cache。
22. 下一步：real Qt login screen、project list UI、SQLite/local cache strategy、packaging strategy spike。
