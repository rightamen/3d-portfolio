...
9. 下一步：真实 HTTP backend / dependency strategy。HTTP backend 必须只消费 `/api/v1/*` strict envelope；JSON parser 需要做长期依赖策略决策（继续最小 parser、或通过未来 C++ dependency manager 引入 header-only parser）；Qt/QML 应在 SDK 边界稳定后接入。
