...
- `cpp-app/` — C++ cross-platform prototype skeleton（CMake + SDK headers + smoke CLI + mock-driven SDK contract tests），已创建；当前已有 opt-in Qt/QML desktop shell 与 mock auth UI flow 第一批，不含真实登录、项目列表、缓存、下载器或打包实现。C++ SDK strict envelope parser boundary 已将 nlohmann/json 设为默认 backend，temporary parser 仍保留为显式 no-dependency fallback。
...
6. C++ SDK model 头文件草案（从 §16 冻结表直接映射）。✅ 已创建第一批可编译头文件骨架：`cpp-app/sdk/models/*.hpp`、`sdk/core/ApiResult.hpp`、`ApiClient`/各 client stub；SDK public surface 不含 admin-only model。
...
21. Qt/QML desktop shell 第一批。✅ 已新增 opt-in `MRRIGHT_ENABLE_QT_UI=OFF` CMake option 和 `cpp-app/app/ui/qt` shell：`main_qt.cpp` 创建 `QGuiApplication` / `QQmlApplicationEngine`，`Main.qml` 显示 app name、SDK version、`/api/v1` strict mode 和 no-network status，`AppController` 暴露只读 shell 信息。SDK core 继续 Qt-free，Qt/QML 层只在 `app/ui/qt`。
22. Qt/QML mock auth flow 第一批。✅ 已扩展 `AppController` UI-only 状态（`isLoggedIn`、`currentUserLabel`、`loginMessage`）和 mock-only `mockLogin` / `logout` / `clearMessage`；`Main.qml` 新增 email/password mock login form，登录后仅更新 UI 状态，logout 后回到未登录。mock flow 不调用 `AuthSession`、不创建 `CurlHttpClient`、不读取或写入 TokenStore、不保存 token、不访问 API、不调用 admin、不做 cache。
23. Qt AppController mock auth tests。✅ 已新增 opt-in `mrright_qt_appcontroller_tests`，仅在 `MRRIGHT_ENABLE_QT_UI=ON` 时构建并注册 CTest；覆盖 initial state、mock login success、validation failure、logout、clearMessage 与 password 非 property 暴露。测试使用 `QCoreApplication`，不启动 GUI、不访问网络、不读取或写入 TokenStore、不保存 token，SDK core 继续 Qt-free。
24. Qt UI AuthService boundary 第一批。✅ 已在 `app/ui/qt` 新增使用 Qt 类型的 `AuthService` 接口和无网络 `MockAuthService`；`AppController` 默认创建 mock service，也支持注入 fake/future adapter，并通过 service 完成 login、logout、message clearing 与状态读取，不再硬编码完整 mock auth 逻辑。测试直接覆盖 `MockAuthService`，并覆盖 controller dependency injection、delegation、property synchronization、notify signals、password/token 非 property 暴露。本批不调用真实 `AuthSession`、不访问 production/local API、不创建 `CurlHttpClient`、不读取或写入 TokenStore、不保存 token/password，SDK core public API 继续 Qt-free。
25. Qt AuthSession adapter 第一批。✅ 已新增 Qt-layer `AuthSessionService` 并实现现有 `AuthService` boundary；adapter 拥有 injected `HttpClient` / `TokenStore` 的生命周期并复用 SDK `AuthSession`，signed-in state 直接来自 `AuthSession::hasSession()`。`mrright_qt_authsession_service_tests` 只使用 `MockHttpClient` + `MemoryTokenStore`，覆盖 strict `/api/v1/auth/*` request construction、login/error/logout/existing-session/message behavior、token placement 与 AppController polymorphic injection。Qt shell 默认仍使用 `MockAuthService`，未启用真实 network backend，SDK core 继续 Qt-free。
26. 下一步：asynchronous auth service API、local-only real login integration、platform SecureTokenStore injection、project list UI、SQLite/local cache strategy、packaging strategy spike。
