...
12. 下一步：libcurl backend spike / dependency manager decision。HTTP backend 必须只消费 `/api/v1/*` strict envelope；需要明确 vcpkg manifest 或 Conan 路线；Qt/QML 应在 SDK 边界稳定后接入。
